...
本仓库实现消费者微信小程序、一级代理工作台、总部管理后台，以及共享的 NestJS API 和 Worker。当前开发按 `B0` 至 `B19` 小批次推进；B0 至 B8 已完成 development 验收。B8 最终实现 SHA `0fc5a8d3d1f07d3b5c9fcadf7ea4ca9560a0911a` 的 [普通 CI Run 33141704459](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33141704459) 和 [Supabase rollback-only Run 33142971501](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33142971501) 为同一 SHA 双绿，B8 development `GO`，CH-017 已失效。当前基线为 `v2.4.7 / CH-020`；B9.0 已在 SHA `97d5f979e63994c4fc136217d870a726f92b3d93` 取得 [普通 CI Run 33151986949](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33151986949)、[Supabase development migration Run 33153469189](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33153469189) 和 [rollback-only smoke Run 33153691333](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33153691333) 三项成功证据。B9.1 结算报价、B9.2 待付款下单/库存预占、B9.3 本人订单查询/主动取消/超时释放和 B9.4 小程序 MP-08/10/11 均已完成；B9.4 经复审达到 `P0=0/P1=0` 后暂停。B9.5 未开始，等待用户批准。B9 development 尚未 `GO`，CH-019 仍有效。仅允许 Mock Provider 和脱敏 development，staging、production、真实客户数据、真实微信身份和资金链路继续 `NO-GO`。
...
Supabase 项目创建、连接分权和受保护烟测见 [B0 工程与 Supabase](product-materials/docs/05-开发管理/B0-工程与Supabase.md)，公共内核边界见 [B1 平台公共内核](product-materials/docs/05-开发管理/B1-平台公共内核.md)，总部认证实现与安全操作见 [B2 总部安全入口](product-materials/docs/05-开发管理/B2-总部安全入口.md)，B3-B8 历史批次见对应开发记录；B9 的 CH-019/CH-020、0002 前向索引迁移、串行批次和准入边界见 [B9 订单报价与库存预占](product-materials/docs/05-开发管理/B9-订单报价与库存预占.md)。普通 PR 只使用 CI 的临时 PostgreSQL，不读取 Supabase 凭据。

B3.1 至 B3.3 的文件、品牌/分类与 ADM-05/06 证据继续有效；B4 已交付 Product/SKU，B5 已交付 Banner/库存，B6 已交付匿名 Store API、MP-01 至 MP-06，B7 已交付消费者身份、资料手机号、归因/服务代理、同步注销及小程序身份页面，B8 已交付收藏、服务端购物车、游客 merge、地址、小程序购物入口和最终双绿。B9.0 已完成契约、0002 加法索引、静态交互和受控远端迁移验收；B9.1 已交付 Store Checkout 报价，B9.2 已交付 Store Order 待付款下单、地址/归因快照、库存预占和 HASH_ONLY 幂等，B9.3 已交付本人订单列表/详情、主动取消和数据库时间驱动的超时释放 Worker，B9.4 已交付结算、订单列表/详情、取消及丢包恢复的小程序实现。B9.5 尚未开始，支付继续延后到 B10。
