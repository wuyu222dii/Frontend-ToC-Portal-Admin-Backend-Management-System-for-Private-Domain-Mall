...
本仓库实现消费者微信小程序、一级代理工作台、总部管理后台，以及共享的 NestJS API 和 Worker。B14 development `GO` 的最终同 SHA migration/smoke 证据保持有效，CH-031 已失效；B15.1 的 `0007`、READY 文件回收与佣金名称快照底座已完成并暂停复审，产品/API 基线为 `v2.4.13 / CH-032`、OpenAPI `2.4.13-ch032`。CH-033 仅覆盖 B15.1-B15.4 单维护者脱敏 development；其余 P2 在实现闭合前继续阻断 staging、production、真实客户数据及真实微信/资金/物流。
...
B3-B14 的历史证据继续以各阶段记录为准。B14 最终实现 SHA `f4521d4188cb74c3bac34b992016d8730468ae2a` 的 development migration `33960486774` 与随后 rollback-only smoke `33960618178` 同 SHA、依次成功；未把未运行的全仓回归或纵向测试记为通过。B15.1 当前已完成并暂停复审；B15.2 尚未准入。
