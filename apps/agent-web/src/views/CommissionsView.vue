  ...
  <AgentShell>
    ...
    <section class="panel">
      ...
      <PageState testid="commissions-state" :empty="!data?.items.length" empty-message="暂无佣金流水" :error="errorMessage" :loading="loading" @retry="load">
        <el-table class="data-table desktop-table" :data="data?.items"><el-table-column label="成交用户" min-width="150"><template #default="scope"><strong>{{ scope.row.customer_alias }}</strong><br><small>{{ scope.row.order_no }}</small></template></el-table-column><el-table-column label="成交商品" min-width="180"><template #default="scope"><strong>{{ scope.row.product_name }}</strong><br><small>{{ scope.row.sku_name }}</small></template></el-table-column><el-table-column label="成交金额" min-width="120"><template #default="scope">{{ formatMoney(scope.row.commission_base) }}</template></el-table-column><el-table-column label="最终分成" min-width="120"><template #default="scope">{{ formatMoney(scope.row.original_commission) }}<br><small>{{ stateLabels[scope.row.position_state] }}</small></template></el-table-column><el-table-column fixed="right" label="操作" width="100"><template #default="scope"><el-button link type="primary" :data-testid="`commission-detail-${scope.row.commission_snapshot_id}`" @click="openDetail(scope.row.commission_snapshot_id)">解释</el-button></template></el-table-column></el-table>
        <div class="mobile-list"><button v-for="item in data?.items" :key="item.ledger_id" class="mobile-record" :data-testid="`commission-detail-${item.commission_snapshot_id}`" type="button" @click="openDetail(item.commission_snapshot_id)"><span class="record-head"><strong>{{ item.customer_alias }}</strong><span class="status-pill">{{ stateLabels[item.position_state] }}</span></span><span class="record-line"><span>{{ item.product_name }} · {{ item.sku_name }}</span><b>{{ formatMoney(item.commission_base) }}</b></span><span class="record-line"><span>{{ item.order_no }}</span><b>分成 {{ formatMoney(item.original_commission) }}</b></span></button></div>
