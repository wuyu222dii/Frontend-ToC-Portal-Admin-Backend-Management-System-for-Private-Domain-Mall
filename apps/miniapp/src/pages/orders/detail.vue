...
const canCancel = computed(() => order.value?.available_actions.includes('CANCEL') === true &&
  remainingSeconds.value > 0 && !cancelPending.value);
...
async function performCancel() {
  ...
  message.value = '';
  try {
    await cancelStoreOrder(current.order_id, current.version, cancelCommand.idempotencyKey);
    cancelCommand = null;
    await loadOrder('订单已取消，预占库存已释放。');
  } catch (error) {
    if (error instanceof StoreApiError && error.status === 401) {
      cancelCommand = null;
      requireLogin();
...
<template>
  <QxStoreShell>
    <view class="order-detail-page">
      ...
      <view
        v-if="state === 'ready' && order && order.available_actions.includes('CANCEL')"
...
.order-detail-actions { position: fixed; z-index: 28; right: 0; bottom: 0; left: 0; display: grid; width: 100%; max-width: 414px; min-height: calc(108rpx + env(safe-area-inset-bottom)); grid-template-columns: minmax(0, 1fr) 176rpx; align-items: center; gap: 14rpx; margin: 0 auto; padding: 12rpx 20rpx calc(12rpx + env(safe-area-inset-bottom)); border-top: 1px solid var(--qx-store-line); background: rgba(255,255,255,.98); }
.order-detail-actions > text { min-width: 0; color: var(--qx-store-muted); font-size: 19rpx; line-height: 1.4; overflow-wrap: anywhere; }
.order-detail-actions button { min-height: 72rpx; border: 1px solid var(--qx-store-danger) !important; border-radius: 9rpx; color: var(--qx-store-danger); background: #ffffff; font-size: 21rpx; font-weight: 750; }
.order-detail-actions button[disabled] { color: var(--qx-store-muted); border-color: var(--qx-store-line) !important; }
@media (max-width: 359px) { .order-status-panel { padding: 22rpx; } .order-detail-actions { grid-template-columns: minmax(0, 1fr) 150rpx; padding-inline: 14rpx; } }
</style>
