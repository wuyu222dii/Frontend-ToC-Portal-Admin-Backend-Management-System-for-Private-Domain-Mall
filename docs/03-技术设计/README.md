...
> 当前基线：MVP/PRD v2.4、CH-001 至 CH-005（2026-08-12）。本目录是开发前技术契约，不包含可部署业务代码。
...
当前无业务工程根目录，因此设计验证 config 暂放在本目录，CLI 必须显式使用 `--config ./prisma.config.ts`。正式 NestJS 脚手架创建后，将 config 迁至工程根目录；CLI/migration 通过 `DIRECT_URL` 连接，runtime 通过 `@prisma/adapter-pg` 读取 `DATABASE_URL`。三端只调用 NestJS HTTPS API，禁止使用 Supabase client、Data API 或 `service_role` 密钥访问业务表。
