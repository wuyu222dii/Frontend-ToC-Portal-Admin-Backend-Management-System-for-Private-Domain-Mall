...
|---|---|
| 文档版本 | v2.4 |
| 对应基线 | MVP/PRD v2.4、CH-001 至 CH-005 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | CH-005 整改稿；首版 migration 已在全新本地 PostgreSQL 18.3 空库回放验证，上线前仍须在 staging 复现 |
| 更新时间 | 2026-08-12 |
...
| `file_asset` | 商品图、Logo/Icon、Banner、售后、付款凭证和推广 QR | `purpose`、创建人、公共/私有权限、对象存储 key、哈希、MIME；只有商品图/品牌 Logo/分类图标/Banner 可 PUBLIC |
