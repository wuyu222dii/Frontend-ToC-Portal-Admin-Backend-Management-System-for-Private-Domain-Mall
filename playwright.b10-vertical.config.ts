...
if (process.env.B10_VERTICAL_TEST_MODE !== 'full') {
  throw new Error('B10 vertical Playwright requires B10_VERTICAL_TEST_MODE=full');
}
const apiPort = process.env.B10_VERTICAL_API_PORT?.trim() || '3000';
const workerPort = process.env.B10_VERTICAL_WORKER_PORT?.trim() || '3001';
const webPort = process.env.B10_VERTICAL_WEB_PORT?.trim() || '5173';
...
export default defineConfig({
  ...
  testMatch: 'b10-store-payments.vertical.spec.ts',
  fullyParallel: false,
  forbidOnly: true,
  reporter: 'line',
  retries: 0,
  ...
  projects: [
    { name: 'chromium-b10-vertical', use: { viewport: { width: 390, height: 844 } } },
  ],
  use: {
    baseURL: webOrigin,
