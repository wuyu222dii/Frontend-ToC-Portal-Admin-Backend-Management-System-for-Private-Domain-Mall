...
| 接口阶段 | B0-B12 development 已完成并维持 `GO`。CH-028 已批准；OpenAPI 基线为 `2.4.11-ch028`，本地实测 `173 paths / 198 operations / 198 unique operationId / 327 schemas / 716 schema refs / 2,746 local refs / 0 dangling refs`，Redocly 0 warning，generated contracts SHA-256 为 `2207286938dd174c0d6b36879407131253ef1ad4ffe6dc3e5726a83827852448`，完整 manifest SHA-256 仍为 `128509f76f2e62ebe78cd0465205a94236f87f70904bf614664dd1b02548dc80`。B13.0 最终 SHA `250f4c8fabb08e170c012889c392461794ed8875` 的普通 CI `33615918343`、development migration `33618959837` 与 rollback-only smoke `33619152221` 已按顺序同 SHA 三绿，B13.0 历史已退出。截至 2026-09-03，B13.1 总部代理生命周期与 7 个 Agent 认证 operation 的本地候选已完成并暂停复审；远端同 SHA 门禁尚未产生，B13.2 未准入，其余 Agent/Admin 业务 operation 仍未开放，B13 development 未标记 `GO`。B12 orphan `P2=1` 继续阻断 staging/真实数据；真实客户数据、staging、production、真实微信退款与真实物流集成均为 `NO-GO` |
...
B13.1 本地候选已实现上表 7 个认证接口。`/agent/**` 只接受独立 Agent realm：`aud=qingxu-agent-web`、`role=AGENT_ADMIN`、`assurance=PASSWORD`；临时密码会话固定 `CHANGE_PASSWORD_ONLY` 且无 refresh。密码变更、refresh 轮换/旧 token replay、当前会话/全部会话撤销、停用代理与登录限流/账号枚举均已有本地回归。创建代理与重置密码仅在首次响应披露秘密；同幂等键重放只返回脱敏资源和重新签发指引，不持久化或再次披露明文。该结论仅支持 B13.1 本地候选；B13.2 业务路由未准入。
