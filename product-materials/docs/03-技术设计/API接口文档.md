...
- CH-024 专属检查、生成与 Redocly lint 已通过，实测为 173 paths、198 operations、198 unique operationId、326 schemas、705 schema refs、2,695 local refs 和 0 dangling refs。专项门禁覆盖全部履约 ID 为 ULID、四个写操作的 `If-Match`/`HASH_ONLY`、个性化 no-store、四个闭合错误码、唯一单包裹、精确发货项、单向物流状态机、两种人工完成原因及佣金一次结转。B11.1 已实现 Admin 订单列表/详情/受控地址与 Store 本人订单物流投影；B11.2 已实现 Admin 发货、STATUS/纠错与 Store 本人物流，完成锁内 SUPER_ADMIN 重验、HASH_ONLY 重放、版本冲突、no-store 和错误矩阵验证。API 聚焦测试为 5 files / 136 passed，最终复审 `P0=0/P1=0/P2=1`，当前暂停在 B11.3 前。唯一 P2 为 CH-024 未冻结相邻 STATUS 事件时间单调性。B11 总体不得 `GO`，最终远端同 SHA 双绿留在 B11.5。
