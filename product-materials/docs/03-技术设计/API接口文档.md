...
| 接口阶段 | B0-B10 development 已完成并维持 `GO`；B11.0-B11.5 本地实现与门禁已完成，包括严格解码、资源绑定、五视口、真实纵向与全仓门禁。OpenAPI 基线为 `2.4.9-ch024`，实测统计保持 173 paths / 198 operations / 198 unique operationId / 326 schemas / 705 schema refs / 2,695 local refs / 0 dangling refs。最终远端同 SHA 普通 CI/Supabase rollback-only 双绿待补，B11 尚未 `GO`，CH-025 仍有效。真实客户数据、staging、production、真实支付与真实物流集成均为 `NO-GO` |
...
- CH-024 专属检查、生成与 Redocly lint 已通过，统计保持 `173/198/198/326/705/2,695/0`。B11.1-B11.4 历史实现证据保持；B11.5 本地真实纵向、严格客户端、API 回归与全仓门禁已完成。API 全量为 **81 files passed / 13 skipped；1,095 passed / 44 skipped**。最终远端同 SHA 双绿待补，B11 未 `GO`，CH-025 仍有效。
