...
| 接口阶段 | B0-B10 development 已完成并维持 `GO`；B11.0-B11.3 本地工程门禁已通过，B11.3 Store 确认收货、Admin 兜底完成和共享完成事务已完成，复审 `P0=0/P1=0/P2=2` 并暂停在 B11.4 前。OpenAPI 基线为 `2.4.9-ch024`，CH-024 专属检查、生成与 Redocly lint 已通过，实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 705 schema refs / 2,695 local refs / 0 dangling refs；冻结数据库、Prisma validate、remote `migration diff=0`、workspace typecheck 与静态原型门禁也已通过。CH-025 已批准且仅适用于 B11.1-B11.5 脱敏 development；B11 总体不得 `GO`，最终远端同 SHA 双绿留在 B11.5。真实客户数据、staging、production、真实支付与真实物流集成均为 `NO-GO` |
...
- CH-024 专属检查、生成与 Redocly lint 已通过，实测为 173 paths、198 operations、198 unique operationId、326 schemas、705 schema refs、2,695 local refs 和 0 dangling refs。专项门禁覆盖全部履约 ID 为 ULID、四个写操作的 `If-Match`/`HASH_ONLY`、个性化 no-store、四个闭合错误码、唯一单包裹、精确发货项、单向物流状态机、两种人工完成原因及佣金一次结转。B11.1 已实现 Admin 订单列表/详情/受控地址与 Store 本人订单物流投影；B11.2 已实现 Admin 发货、STATUS/纠错与 Store 本人物流；B11.3 已实现 Store 确认收货、Admin 兜底完成和共享完成事务，覆盖锁内角色/客户重验、HASH_ONLY 重放、版本冲突、no-store 与错误矩阵。API 全量为 **81 files passed / 13 skipped；1,093 passed / 44 skipped**。B11.2 的既有非阻断 P2 仍只记录 CH-024 未冻结相邻 STATUS 事件时间单调性；B11.3 复审为 `P0=0/P1=0/P2=2`，其 P2 为未来普通全额退款改变完成原因后原 HASH_ONLY 键重放 fail-closed 500。普通退款不在 B11，当前暂停在 B11.4 前。B11 总体不得 `GO`，最终远端同 SHA 双绿留在 B11.5。
