...
| 接口阶段 | B0-B11 development 已完成并维持 `GO`。CH-026 已批准；OpenAPI `2.4.10-ch026` 实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 706 schema refs / 2,726 local refs / 0 dangling refs。B12.0 精确 SHA 三项远端门禁与 CH-027 已闭合；B12.1-B12.4 后端接口及 B12.5 generated contracts/严格响应解码接入已完成并在 B12.5 验收后暂停，B12.5 未改变 API 统计。B12.6 尚未开始，B12 仍非 development `GO`，CH-027 仍有效；真实客户数据、staging、production、真实微信退款与真实物流集成均为 `NO-GO` |
...
- CH-026 契约解析实测为 173 paths、198 operations、198 unique operationId、326 schemas、706 schema refs、2,726 local refs、0 dangling refs，Redocly 0 warning。机器门禁覆盖 18 个售后/退款 operation、2 个金额补偿 operation、Store PREVIEW/CONFIRM 200/201、验货 PASS/ABNORMAL 判别联合、ULID、Store 限流/no-store、HASH_ONLY/If-Match、闭合原因/错误/证据/承运字段、Admin preview-confirm、来源闭合 retry，以及订单售后摘要和完整 `APPLY_AFTERSALE` 矩阵。B12.1-B12.4 已实现 Store 申请/查询/取消、Admin 初审/退货地址/验货与异常处置、普通退款/重试和金额补偿后端；B12.5 已使用 generated contracts 和严格响应解码器接入 MP-13/14、ADM-12/13/16，并建立五视口 E2E。OpenAPI、统计和 generated contracts 契约形状未因 B12.5 修改；B12.6 总验收尚未完成，仍不代表 B12 development `GO`。
