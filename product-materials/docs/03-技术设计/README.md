...
> 当前产品/API 基线为 MVP/PRD v2.4.10、在线协议 CH-026；数据库修复 CH-023（2026-08-30）继续有效。B0-B11 development 已完成并维持 `GO`；B12.0 精确 SHA 三项远端门禁与 CH-027 已闭合。B12.1-B12.4 后端已完成并在 `P0=0/P1=0` 后逐批暂停，B12.5-B12.6 尚未开始，B12 development 尚未 `GO`。真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO`。
...
B12 的已批准 CH-026/CH-027、普通售后/验货/普通退款契约、0005 防护和 B12.0-B12.6 批次见 `../05-开发管理/B12-售后验货与普通退款.md`。B12.0 三项远端证据已闭合；B12.1-B12.4 后端已完成并逐批暂停，B12.5-B12.6 尚未开始。不得把 B12.4 描述为工程前端、总验收或 B12 development `GO`。
