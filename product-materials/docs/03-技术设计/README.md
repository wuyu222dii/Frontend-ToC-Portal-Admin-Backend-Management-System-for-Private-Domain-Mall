...
> 当前产品/API 基线为 MVP/PRD v2.4.9、在线协议 CH-024；数据库修复 CH-023（2026-08-30）继续有效。B0-B10 development 已完成并维持 `GO`；B11.0 与 B11.1 本地门禁已通过，B11.1 `P0=0/P1=0` 并暂停在 B11.2 前。CH-025 已批准且仅适用于 B11.1-B11.5 脱敏 development，继续有效至 B11.5 完成；B11 总体不得 `GO`，尚无最终远端同 SHA 双绿。真实客户数据、staging、production、真实支付与真实物流均为 `NO-GO`。
...
- CH-024 数据库变更数固定为 0：两份 Prisma schema 与 `0001` 至 `0004` 必须逐字节不变，完整迁移链、冻结哈希、权限、Prisma validate 和 `migration diff=0` 是 B11 全阶段强制门禁。B11.1 已完成本地验收且未修改 Prisma 或 migration。
...
B11 的已批准 CH-024、冻结履约状态机、零迁移边界及批次见 `../05-开发管理/B11-订单履约与物流.md`。CH-025 已批准；B11.1 已实现独立 Fulfillment 查询、Admin/Store 投影、受控履约地址和共享 `display_status`，本地验收 `P0=0/P1=0` 后暂停在 B11.2 前。B11 总体不得 `GO`，B11.2 及后续批次仍须逐批明确准入。
