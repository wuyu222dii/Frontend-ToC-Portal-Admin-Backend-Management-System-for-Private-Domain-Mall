...
> 技术基线：产品/API 为 v2.4.10 / CH-026；数据库修复 CH-023（2026-08-30）继续有效。状态：B0-B11 development 已完成并维持 `GO`；B12.0 精确 SHA 三项远端门禁与 CH-027 已闭合。B12.1-B12.4 后端已完成并在 `P0=0/P1=0` 后逐批暂停，B12.5-B12.6 尚未开始，B12 development 尚未 `GO`。真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO`
...
- B12.0 精确 SHA `03a1e9f98bea0926887fb68354ca602566148f6a` 已依次通过普通 CI、Supabase development migration 和 rollback-only smoke，CH-027 已批准。B12.1-B12.4 已实现并在 `P0=0/P1=0` 后逐批暂停；B12.4 已交付普通退款/金额补偿 preview-confirm、Mock Provider/Inbox/Worker、稳定失败重试和库存/订单/佣金钱包收敛，B12.5 工程前端尚未开始。
