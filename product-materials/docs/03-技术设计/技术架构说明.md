...
> 技术基线：产品/API 为 v2.4.11 / CH-028；数据库修复 CH-023（2026-08-30）继续有效。状态：B0-B12 development 已完成并维持 `GO`；B12 最终 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿，CH-027 已自动失效。B13.0 最终 SHA `250f4c8fabb08e170c012889c392461794ed8875` 的普通 CI `33615918343`、development migration `33618959837` 与 rollback-only smoke `33619152221` 已按顺序同 SHA 三绿并退出。截至 2026-09-03，B13.1 最终 SHA `a565a3c406ad2f1af498f9f58b556315d30262d6` 的普通 CI `33704252016`、development migration `33706364405` 与 rollback-only smoke `33706498758` 同 SHA 三绿，B13.1 已退出，B13.2 本地候选实现与最终复审已完成，复审结论为 `P0=0/P1=0/P2=0`，正在等待本批最终同 SHA 普通 CI -> development migration -> rollback-only smoke；B13.2 未退出；B13.3 未准入；B13 development 仍非 `GO`。佣金/钱包/提现业务实现仍待后续批次。B12 orphan `P2=1` 继续阻断 staging/真实数据；真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO`
...
- B12.0 精确 SHA `03a1e9f98bea0926887fb68354ca602566148f6a` 已依次通过普通 CI、Supabase development migration 和 rollback-only smoke。B12.1-B12.6 本地实现与本地验收已完成，覆盖普通退款/金额补偿 preview-confirm、Mock Provider/Inbox/Worker、稳定失败重试、库存/订单/佣金钱包收敛、工程前端、完整真实纵向及精确清理；最终实现 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿，B12 development `GO`，CH-027 已自动失效。B13.0 已在 SHA `250f4c8fabb08e170c012889c392461794ed8875` 取得同 SHA 三绿并退出；B13.1 最终 SHA `a565a3c406ad2f1af498f9f58b556315d30262d6` 已取得同 SHA 三绿并退出，B13.2 本地候选实现与最终复审已完成，复审结论为 `P0=0/P1=0/P2=0`，正在等待本批最终同 SHA 普通 CI -> development migration -> rollback-only smoke；B13.2 未退出；B13.3 未准入。
...
- CH-026 契约实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 706 schema refs / 2,726 local refs / 0 dangling refs，Redocly 0 warning。`0005` 本地完整五迁移回放、22 个 partial indexes、16 个自有函数、runtime 合法退款项写入、非法包络/金额拒绝、父 refund 并发串行和 `migration diff=0` 已通过；B12.0 远端三项证据也已闭合。B12.1-B12.6 本地实现保持 OpenAPI、Prisma 与既有迁移冻结；完整 `RETURN_REFUND` 真实纵向和精确清理已通过。最终实现 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿，B12 development `GO`，CH-027 已自动失效；B13.0-B13.1 已退出，B13.2 本地候选实现与最终复审已完成，复审结论为 `P0=0/P1=0/P2=0`，正在等待本批最终同 SHA 普通 CI -> development migration -> rollback-only smoke；B13.2 未退出；B13.3 未准入，B13 development 仍非 `GO`。
