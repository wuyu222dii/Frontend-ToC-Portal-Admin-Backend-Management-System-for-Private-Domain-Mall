...
> 技术基线：v2.4.2 / CH-010（2026-08-24）；状态：B4.0 Product/SKU 契约解阻实施中，B4.1 等待契约门禁与准确 SHA 普通 CI 全绿；staging/production 未批准
...
| Catalog | 品牌、一级分类与素材；ProductCatalog 子模块负责商品、SKU、图集和只读库存投影；Banner 后续独立交付 | 不直接修改历史订单快照，不在商品编辑中写库存 |
...
- Product/SKU 状态矩阵、首次 `published_at`、nullable 最低活动价、归档 SKU、8 图、零库存初值、不可变 code 和四个 422 已纳入契约门禁；对应事务与并发测试必须在 B4.1/B4.2 补齐，未通过不得进入下一批。
- Prisma/schema/`0001_initial` 逐字节不变，Prisma validate 与 migration diff 为零；不通过时停止 B4.0。
- B3 验收及 Supabase development rollback-only 已在实现基准 SHA 的 GitHub Run `32678252828` 通过。B4.1 仍须等待 CH-010 契约门禁和准确 SHA 的普通 CI 全绿；本地/临时 PostgreSQL 与 MinIO 证据不得替代远端记录，CH-009 不放行 staging/production。
