...
> 技术基线：v2.4.2 / CH-010（2026-08-24）；状态：B4.0 至 B4.4 development 已完成，同一实现 SHA 的普通 CI 与 Supabase rollback-only 均通过；staging/production 未批准
...
- B4 实现基准 SHA `0929f2435e7f5b9ad745fd9cab60b066378e502e` 的普通 CI Run `32721588213` 与 Supabase development rollback-only Run `32722510890` 均成功。本地/临时 PostgreSQL 与 MinIO 证据未被用于替代远端记录。CH-009 已在 B4 development 结束时耗尽；CH-011 仅接续 B5 development 的单人维护补偿控制，不批准 CH-012，也不放行 staging/production。
