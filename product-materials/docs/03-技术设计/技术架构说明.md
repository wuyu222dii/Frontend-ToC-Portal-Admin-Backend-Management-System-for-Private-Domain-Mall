...
> 技术基线：v2.4.5 / CH-016（2026-08-27）；状态：B0-B6 development 已完成，CH-015/CH-016 已批准；B7.0-B7.2 已完成并按批暂停，B7.2 `P0=0/P1=0`，B7.3 未准入；仅允许 Mock Provider 和脱敏 development，staging/production 未批准
...
6. Catalog 路径依次锁 `brand -> category -> product -> sku -> inventory_balance/inventory_reservation`，每类按 ID 升序；非 Catalog 交易路径跳过 brand/category/product，并继续按 `sku_id` 升序；
...
- 品牌/分类生命周期与 Product/SKU 发布共享锁序 `brand -> category -> product -> SKU -> inventory reservation`，同类 ID 升序；不得在持有数据库锁时访问对象存储或其他外部 Provider。
...
- 库存 confirm 的锁序为 `idempotency -> high_risk_preview -> sku -> inventory_balance -> reservation IDs -> inventory_ledger -> audit/outbox`。同一串行化事务重查版本和活动预占，条件更新 physical/version，插入一条人工流水并消费 preview；任何 409/422 都整体回滚且不消费 preview。
...
- 候选创建只接收邀请码和推广物料 ID，服务端解析 target；token 仅存用途隔离的 HMAC/哈希，和幂等键、来源 IP 分别使用不同 domain/scope，固定 30 分钟有效。GET 查询不消费 token，候选替换或登录迁移原子失效 token hash；确认/拒绝消费迁移后的候选事实。确认锁序为 `idempotency -> customer_profile -> candidate -> current binding -> agent/invite/promotion validity -> audit/outbox`，与总部转移/订单归因共用 customer_profile 串行根。
- 服务代理使用独立三字段投影。注销 preview 不合格时只返回 blockers/impacts，不签发 token；合格时签发绑定 CUSTOMER/session/account version 的 5 分钟能力。同步 confirm 按 `idempotency/preview -> account/customer_profile -> blocker facts -> sessions/identity/phone/candidate/binding/privacy projection -> audit/outbox` 单事务执行，重检失败或任一步异常整体回滚。
- 注销 confirm 使用 HASH_ONLY，不缓存或重放完成响应；事务提交后旧 bearer/refresh 全部失效，重复请求只返回认证失效或已完成冲突。现有 Prisma/首迁移足以承载全部事实，CH-016 结构变更数固定为 0。
...
- 限流使用 `STORE_LEGAL_RATE_LIMIT_MAX=120`、`STORE_LEGAL_RATE_LIMIT_WINDOW_SECONDS=60`、`STORE_LOGIN_RATE_LIMIT_MAX=10`、`STORE_LOGIN_RATE_LIMIT_WINDOW_SECONDS=900`；均使用 Redis 服务端时间并 fail closed。
...
- CH-016 专项实测为 173 paths / 197 operations / 197 unique operationId / 320 schemas / 699 schema refs / 2,617 local refs / 0 dangling refs，Redocly 0 warning。三份法律文本、Store/Admin audience 隔离、服务端 Provider 选择、If-Match、候选 token/目标、服务代理最小投影和同步注销已纳入 B7.0 契约门禁；B7.1 已完成 Store 身份与会话，B7.2 已完成资料与账户手机号的双主体绑定、If-Match/HASH_ONLY、AES-256-GCM AAD、独立域 HMAC 密钥环、Mock Provider fail closed、重授权/撤回、并发与 full/rollback 门禁，退出审查 `P0=0/P1=0`。B7.3 尚未准入。
- Prisma/schema/`0001_initial` 逐字节不变，Prisma validate 与 migration diff 为零。
- B5/B6 历史同 SHA 双绿证据继续有效。CH-015 只接续 B7.0-B7.5 脱敏 development 的单人维护补偿控制，将随 B7.5 自动失效，且不放行 staging/production。
