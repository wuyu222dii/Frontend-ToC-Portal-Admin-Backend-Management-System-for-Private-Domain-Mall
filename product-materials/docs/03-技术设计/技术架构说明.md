...
> 技术基线：产品/API 为 v2.4.10 / CH-026；数据库修复 CH-023（2026-08-30）继续有效。状态：B0-B11 development 已完成并维持 `GO`；B12.0 精确 SHA 三项远端门禁与 CH-027 已闭合。B12.1 最小 Store 售后后端已完成并在 `P0=0/P1=0` 后暂停，B12.2 尚未开始，B12 development 尚未 `GO`。真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO`
...
- B12.0 精确 SHA `03a1e9f98bea0926887fb68354ca602566148f6a` 已依次通过普通 CI、Supabase development migration 和 rollback-only smoke，CH-027 已批准。B12.1 已实现最小 Store 售后后端并在 `P0=0/P1=0` 后暂停；验货、退货物流、普通退款和管理端处置仍由 B12.2 以后批次实施。
...
- CH-026 契约实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 706 schema refs / 2,720 local refs / 0 dangling refs，Redocly 0 warning。`0005` 本地完整五迁移回放、22 个 partial indexes、16 个自有函数、runtime 合法退款项写入、非法包络/金额拒绝、父 refund 并发串行和 `migration diff=0` 已通过；B12.0 远端三项证据也已闭合。B12.1 已实现并暂停，B12.2 尚未准入。
