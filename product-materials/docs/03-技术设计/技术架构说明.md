...
> 技术基线：v2.4.3 / CH-012（2026-08-25）；状态：B5.1 Banner repository/API 已通过本地及一次性 PostgreSQL 验收并暂停，B5.2 尚未开始，staging/production 未批准
...
| Catalog | 品牌、一级分类与素材；ProductCatalog 子模块负责商品、SKU、图集和只读库存投影；Banner 作为 B5 独立子模块交付 | 不直接修改历史订单快照，不在商品编辑中写库存 |
...
- CH-012 的 Banner 闭合状态机、公开投影、URL allowlist、库存统一公式、HR-07 preview-confirm、int32 边界、闭合流水类型和幂等策略已通过 B5.0 契约门禁；实测为 172 paths / 196 operations / 196 unique operationId / 312 schemas / 692 schema refs / 2,578 local refs / 0 dangling refs。B5.1 已实现独立 repository/API、稳定锁序、一致性公开投影、audit/outbox 和精确重放并暂停，B5.2 尚未开始。
- Prisma/schema/`0001_initial` 逐字节不变，Prisma validate 与 migration diff 为零。
- B4 实现基准 SHA `0929f2435e7f5b9ad745fd9cab60b066378e502e` 的普通 CI Run `32721588213` 与 Supabase development rollback-only Run `32722510890` 均成功。本地/临时 PostgreSQL 与 MinIO 证据未被用于替代远端记录。CH-011 仅接续 B5 development 的单人维护补偿控制；B5.1 的一次性 PostgreSQL full 证据不替代 B5.4 的准确 SHA 普通 CI 与 Supabase rollback-only，也不放行 staging/production。
