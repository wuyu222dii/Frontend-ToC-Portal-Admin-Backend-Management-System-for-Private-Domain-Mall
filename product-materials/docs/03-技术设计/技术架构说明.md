...
> 技术基线：v2.4.6 / CH-018（2026-08-28）；状态：B0-B7 development 已完成；CH-017/CH-018 已批准，B8.0-B8.5 本地实现与验收已完成并暂停，最终同一 SHA 的普通 CI 与 Supabase rollback-only 双绿待取得，B8 整体尚未 `GO`，CH-017 继续有效；仅允许 Mock Provider 和脱敏 development，staging/production 未批准
...
- CH-018 专项实测为 173 paths / 198 operations / 198 unique operationId / 323 schemas / 701 schema refs / 2,653 local refs / 0 dangling refs，Redocly 0 warning。收藏、服务端购物车、游客合并、地址与小程序已实现；B8.5 已加入数据库 full/rollback、真实浏览器纵向及普通 CI，总验收本地结论为 `P0=0/P1=0/P2=0`，仅待最终同一 SHA 远端双绿。
