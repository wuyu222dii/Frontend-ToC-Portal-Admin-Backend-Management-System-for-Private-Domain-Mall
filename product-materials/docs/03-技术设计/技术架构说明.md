...
> 技术基线：v2.4.1 / CH-006（2026-08-24）；状态：B3 后端与管理端 development 已实现并验收 `GO`，CH-009 允许 B4 development；staging/production 未批准
...
- B3.0 验收后曾暂停；B2/B3 Supabase development rollback-only 后续已在实现基准 SHA 的 GitHub Run `32678252828` 通过。本地/临时 PostgreSQL 和 MinIO 证据仍不得替代远端记录；CH-009 只放行后续 development，不放行 staging/production。
