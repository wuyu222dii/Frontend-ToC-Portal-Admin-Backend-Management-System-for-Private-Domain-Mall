...
> 技术基线：v2.4.6 / CH-018（2026-08-27）；状态：B0-B7 development 已完成，B7 最终 SHA `3f844bfb9866854ceedb975ad0dc4fd7cacfb04a` 的普通 CI Run `33055090596` 与 Supabase development rollback-only Run `33056078437` 双绿，B7 development `GO`，CH-015 已失效；CH-017/CH-018 已批准，当前只完成 B8.0 契约与治理并暂停，B8.1 业务代码尚未开始；仅允许 Mock Provider 和脱敏 development，staging/production 未批准
...
- CH-018 复用现有 `favorite/cart/cart_item/customer_address`、幂等、审计和密钥环结构，Prisma 与两份首迁移逐字节不变，结构变更数固定为 0。B8.0 只冻结契约与静态原型，B8.1 repository/API 尚未开始。
...
- CH-018 专项实测为 173 paths / 198 operations / 198 unique operationId / 323 schemas / 701 schema refs / 2,653 local refs / 0 dangling refs，Redocly 0 warning。收藏失效投影、购物车 selected/五状态/merge、地址加密/默认/If-Match、HASH_ONLY、no-store 与登录后 120/60 fail-closed 限流均已形成机器门禁；B8.0 完成后暂停，B8.1 尚未开始。
- Prisma/schema/`0001_initial` 逐字节不变，Prisma validate 与 migration diff 为零。
- B5/B6/B7 历史同 SHA 双绿证据继续有效。CH-017 只接续 B8.0-B8.5 脱敏 development 的单人维护补偿控制，将随 B8.5 自动失效，且不放行 staging/production。
