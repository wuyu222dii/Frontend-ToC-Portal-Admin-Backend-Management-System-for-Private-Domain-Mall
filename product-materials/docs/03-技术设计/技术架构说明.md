...
> 技术基线：产品/API 为 v2.4.9 / CH-024；数据库修复 CH-023（2026-08-30）继续有效。状态：B0-B10 development 已完成并维持 `GO`；B11.0 与 B11.1 本地门禁已通过，B11.1 `P0=0/P1=0` 并暂停在 B11.2 前。CH-025 已批准且仅适用于 B11.1-B11.5 脱敏 development，继续有效至 B11.5；B11 总体不得 `GO`，尚无最终远端同 SHA 双绿。真实客户数据、staging、production、真实支付与真实物流均为 `NO-GO`
