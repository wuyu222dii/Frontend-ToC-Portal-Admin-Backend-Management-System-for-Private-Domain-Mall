...
> 技术基线：产品/API 为 `v2.4.13 / CH-032`，OpenAPI 为 `2.4.13-ch032`；数据库修复 CH-023 继续有效。B15 development `GO`，B16 准备项已记录；B17.0 正在执行首次脱敏 Mock staging 的候选冻结与证据核验。B17 不新增业务迁移、Worker、队列、依赖或 GitHub CI；B12 orphan 现场回收、恢复演练和外部复核完成前，staging、production、真实数据和真实资金仍为 `NO-GO`。
...
- 主数据：腾讯云 TencentDB PostgreSQL；只承担数据库托管，不承接认证、业务授权、对象存储或前端数据 API；当前逻辑模型为 76 张应用表；
...
- 运行库使用腾讯云 TencentDB PostgreSQL；进程拒绝任何 `SUPABASE_*` 变量和 `*.supabase.co` / `*.supabase.com` 主机。
- 小程序、代理端和总部端禁止持有数据库连接，禁止持有 `anon`、`authenticated` 或 `service_role` 密钥。
- 微信授权登录、管理员密码登录、会话、手机号授权和 RBAC 均由现有 Auth/Access Control 模块实现。
- 应用表只放在 `public`。文件仍由 NestJS File 模块接入 S3 兼容对象存储。
- 首迁移创建的无登录 `anon` / `authenticated` / `service_role` 对全部应用表和序列默认无权限且无 RLS policy。应用行级授权仍只在 NestJS 校验，数据库 RLS 仅隔离连接角色。

### 2.2 连接、角色与密钥

| 用途 | 连接模式 | 端口与限制 |
|---|---|---|
| 长驻 API/Worker | TencentDB 内网 | 使用内网域名；允许连接池和 prepared statements |
| Prisma migration、`pg_dump`/恢复 | TencentDB 内网 | 只使用 `mall_migrator` 的 `DIRECT_URL` |

- 每个 API/Worker runtime 除 `DATABASE_POOL_MAX` 主池外，另创建一个同 URL、`max=1` 的协调池供需要在 advisory lock 内继续使用 Prisma 的 owner/Worker 路径使用。协调池不承载普通查询，空闲或已借出连接发生错误时必须 fail closed。
- `mall_runtime` 仅拥有业务运行所需的表级 DML、序列使用权和专用 RLS policy；不得拥有 schema、DDL、角色管理或 `BYPASSRLS` 权限。
- 首次安装由 TencentDB 实例管理员执行角色初始化和基线 migration，随后将 76 张应用表、59 个枚举与迁移函数的所有权移交给 `mall_migrator`。后续 migration 只通过 `mall_migrator` 执行，日常运行不得使用实例管理员、`mall_migrator` 或 `service_role`。
- 按 Prisma ORM 7 连接约定，`schema.prisma` 不保存 URL；`prisma.config.ts` 的 `datasource.url=env('DIRECT_URL')` 只供 CLI/migration 使用，NestJS API/Worker 通过 `@prisma/adapter-pg` 读取 `DATABASE_URL`。两者使用独立最小权限凭据。
- 数据库密码、CA 证书、Provider 密钥和信封加密密钥进入 Secret Manager，不进入前端构建产物、日志或仓库。
- 所有远程数据库连接必须 `sslmode=verify-full` 并使用腾讯云 CA；禁止 `sslmode=disable` 或跳过证书校验。
...
| Fulfillment | B11 独立模块负责唯一单包裹、精确发货项、人工物流节点、消费者确认和总部兜底完成 | 不发出已退款或存在活动售后的订单，不接第三方物流、不自动确认、不拆分多包裹 |
| Aftersale | B12 独立模块负责本人售后申请/查询/取消、额度占用、总部审核、退货验货、普通退款与失败重试 | 不按当前商品价反算历史退款，不在事务内调用 Provider，不让消费者重试退款 |
...
- CH-022 将关闭扩展为三段 Provider 流程：首个短事务 claim `CREATING/OPEN` 为 `CLOSE_PENDING`，事务外按稳定 `intent_no` query/close，明确不可支付后再在最终事务关单释放；未知结果继续保持占用。B10.3/B10.4 的准确 SHA 三项远端证据作为历史批次证据保留；B10.5 小程序支付结果和 ADM-10 最小对账及 B10.6 总验收均已完成，最终 SHA `f5e59169b53a97704711c3aae3049e5b5d16a930` 已取得同 SHA 普通 CI 与 Supabase development rollback-only smoke 双绿。
- ADM-10 的整条 reconcile 命令按 payment intent 获取 owner lock；需要在锁内继续使用 Prisma 的 API/Worker owner 路径使用独立协调池，Callback Inbox 回排事务则使用其持锁连接自身执行。锁获取超时、已借出连接丢失、空闲连接错误或无法确认安全解锁时一律 fail closed，不能把未知执行结果返回为成功。
- Callback Inbox 的 FAILED/PROCESSED 回排与 Worker 消费使用同一 Inbox advisory-lock namespace。终态非成功 intent 已落库但订单未关闭或仍有 ACTIVE 预占时，ADM-10 仍以既有 `PAYMENT_INTENT/CLOSE_PENDING + ORDER_CLOSE_INCOMPLETE` 投影发现它，并只执行本地 Serializable 关单补偿；该路径不再次调用 Provider，只有订单 `CLOSED` 且不存在 ACTIVE 预占才算收敛。
...
- ADM-10 对账在任何 Provider 副作用前校验运行时 Provider 配置和存量 intent locator；配置不一致返回 503，locator 不一致返回 `PAYMENT_RESULT_CONFLICT`，不得 close、回写或完成幂等事实。支付意图 owner lock 使用独立协调池覆盖重读、事务外 Provider I/O 与最终提交；Callback 回排和 Worker 消费按 Inbox 键互斥，锁连接故障不得返回成功。
...
- 终态非成功 intent 与订单/预占发生历史分叉时，任务仍按现有 `PAYMENT_INTENT/CLOSE_PENDING` 类型公开，并以 `ORDER_CLOSE_INCOMPLETE` 标记；内部修复只重验并完成本地关单/释放，不重新触碰 Provider，也不扩展 OpenAPI 枚举。
- `0003_b10_payment_fact_indexes` 只增加“每 intent 最多一个成功 attempt”和“每订单最多一个迟到退款”两个条件唯一索引；迁移前检测历史重复事实。B10.5/B10.6 不新增迁移，完整 `0001→0004`、20 个 partial indexes 和 `migration diff=0` 已在 B10.6 总验收通过；最终 SHA `f5e59169b53a97704711c3aae3049e5b5d16a930` 的[普通 CI Run 33305811318](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33305811318)与[Supabase development rollback-only smoke Run 33306877575](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33306877575)同 SHA 且均成功，B10 development `GO`，CH-021 已自动失效。
...
### 5.13 CH-024 单包裹人工履约边界

- Fulfillment 使用独立 repository/application service，Store 只开放本人物流读取与确认收货，Admin 只开放订单读取、受控履约地址读取、创建单包裹、追加人工物流事件与兜底完成。已有路径不增删；全部订单、订单项和包裹标识在 controller 入口先校验 ULID。
- 支付成功留下 `PENDING_SHIPMENT/READY_TO_SHIP`，发货事务原子创建唯一 shipment 并推进为 `SHIPPING/SHIPPED`。人工 STATUS 事件只能执行 `SHIPPED -> IN_TRANSIT -> DELIVERED`，承运商/运单更正只追加事实，不改变状态。消费者确认或管理员兜底是独立完成命令：允许已支付、NORMAL、`SHIPPING` 且包裹已处于 `SHIPPED|IN_TRANSIT|DELIVERED` 的订单，在无活动售后/未解决支付异常时原子封存 `DELIVERED` 并进入 `COMPLETED`，原因分别固定为 `CUSTOMER_CONFIRMED`、`ADMIN_FORCED`；该路径不伪造跳过的人工物流节点。不存在自动确认、人工状态跳级或倒退。
- 发货采用 Serializable 与统一重试，锁序固定为 `idempotency -> admin account -> order -> order items ASC -> active aftersales ASC -> existing shipment`。事务内重验 `PENDING_SHIPMENT/READY_TO_SHIP`、订单版本、活动售后、唯一包裹和每个订单项的全部剩余可发数量；任一不符整体回滚。写 shipment/items、订单/订单项发货数量、审计、Outbox 与 HASH_ONLY 幂等完成事实后一次提交。
- 人工物流事件先无锁读取 shipment 仅用于定位 order，再按 `idempotency -> admin account -> order -> shipment -> existing events` 加锁与重验；客户端不提交 `event_key`，服务端从幂等事实派生稳定值，`(shipment_id,event_key)` 只作为数据库并发兜底，不能替代请求幂等键或 `If-Match`。状态推进原子更新 shipment/order version 与时间线；纠错事件只追加完整新承运商/运单和原因。
- 两种完成入口复用同一 application service。完成事务按 `idempotency -> actor/customer -> order -> order items ASC -> active refund/aftersale ASC -> shipment -> current PUBLISHED business rule -> commission snapshots/positions ASC -> agent wallets ASC -> audit/outbox` 加锁；锁内重验已支付/NORMAL、订单 `SHIPPING`、包裹已发出、资源版本与无阻断事实。事务原子封存 `DELIVERED`，写 `COMPLETED`、完成原因/时间、`business_rule_version_id`、`aftersale_expires_at`，并把每个 EXPECTED position 的 `expected_remaining` 精确一次结转为 AVAILABLE；零佣金和已退款减少部分不重复入账。
- 确认收货、发货、物流事件与后台完成都使用 `Idempotency-Key + If-Match + HASH_ONLY`，完成重放先重新鉴权并返回当前投影；不得缓存响应正文。Store 履约成功和 JSON 错误、Admin 订单列表/详情/履约地址/发货/物流/完成的 2xx 响应强制 `Cache-Control: no-store, private`、`Pragma: no-cache`；Admin 错误响应不得携带 PII，履约地址继续由独立 PII 权限与逐次审计保护。
- CH-024 不新增表、列、枚举、索引、Redis 状态或 Provider。两份 Prisma schema 和 `0001` 至 `0004` 逐字节不变；B11.1 只增加查询 repository/API 与共享投影，未修改 Prisma 或 migration。多包裹、第三方物流、普通售后与自动确认收货留给后续独立变更。

### 5.14 CH-026 普通售后、验货与退款边界

- B12 复用既有 18 个售后/退款 operation 和 2 个金额补偿 operation。Store 五个接口统一 CUSTOMER+来源 IP `120/60` fail-closed 限流、no-store/private/no-cache；写操作使用 `HASH_ONLY`，已有资源写操作使用 `If-Match`。`POST /store/aftersales` 在同一 operation 内使用 `PREVIEW -> CONFIRM` 闭合命令，两阶段使用不同幂等键；订单投影按 PAID/NORMAL、完成前/完成后期限和逐项剩余额度矩阵权威返回 `APPLY_AFTERSALE`，客户端不得自行推导资格。
- PREVIEW 只提交订单项、数量和其他闭合业务字段，使用 Repeatable Read 按成交快照分配金额，并从现有幂等密钥环经 HKDF 域 `qingxu:store-aftersale-preview:v1` 派生签名密钥，签发绑定客户、会话、规范请求、资源版本、占用/退款、证据及金额的 5 分钟无状态 HMAC；CONFIRM 在 Serializable 事务内重验后才占用。申请与验货证据均最多 9 个不重复 ULID，必须绑定 READY/PRIVATE/AFTERSALE_EVIDENCE 及当前业务主体；退货物流字段使用闭合 ASCII/长度约束。
- 初审、验货、异常解决、普通退款和金额补偿均按固定资源锁序重验。拒绝、验货后拒绝、退款、失败重试及金额补偿使用 preview-confirm；preview token 只存哈希并绑定 actor、session、action、target、请求与版本。
- Provider `refund/query` 一律位于数据库事务外。准备事务创建稳定 refund/attempt，结果先写 Inbox，再由 Worker 以 Serializable 事务收敛退款、占用、订单项、适用库存回库、佣金/钱包、审计和 Outbox；UNKNOWN、失败、丢包、重复或乱序不得推定成功或另建 refund。
- `0005_b12_aftersale_refund_guards` 不增表、列或枚举；它移除退款项触发器对不可变验货行的不必要锁定读取，增加每退款单活动/成功 attempt 的两个条件唯一索引，并以可延迟约束触发器强化 refund/order/customer/source/items 包络与头明细金额相等。既有 `0001` 至 `0004` 保持逐字节不变。
- B12.0 精确 SHA `03a1e9f98bea0926887fb68354ca602566148f6a` 已依次通过普通 CI、Supabase development migration 和 rollback-only smoke。B12.1-B12.6 本地实现与本地验收已完成，覆盖普通退款/金额补偿 preview-confirm、Mock Provider/Inbox/Worker、稳定失败重试、库存/订单/佣金钱包收敛、工程前端、完整真实纵向及精确清理；最终实现 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿，B12 development `GO`，CH-027 已自动失效。B13.0-B13.8 的历史分批退出事实继续有效；B13 最终 SHA `250cbb824ad11f4b0e11338c19494655ec384ff6` 的 development migration `33952031401` 与随后 rollback-only smoke `33952283771` 同 SHA、依次成功，B13 development `GO`，CH-029 已失效。
...
B10 development 只允许 Mock Provider；服务端生成稳定交易号和事件号，Mock 结果入口只写 Inbox。该入口仅在非 production 的 `development|test + MOCK` 注册，`test` 只用于 B10.6 真实 HTTP 纵向；production/WECHAT 始终不注册。正式微信资质就绪后替换 Adapter，不修改订单、退款、佣金和库存领域模型；验签、证书轮换和 ACK 在 staging 单独准入。
...
- API/Worker 使用自建微信认证和应用 RBAC；数据库 RLS 只隔离连接角色，不替代租户、角色或代理数据范围校验。
- 首迁移创建的无登录 `anon`、`authenticated` 和 `service_role` 不进入任何前端或后端应用配置。
...
首期建议两类无状态容器：API 至少 2 副本、Worker 至少 1 副本；PostgreSQL 使用腾讯云 TencentDB，Redis 和对象存储使用腾讯云托管服务。长驻 API/Worker 走 TencentDB 内网域名；migration、`pg_dump` 和恢复使用 `mall_migrator` 的 `DIRECT_URL`。Nginx/云负载均衡终止 HTTPS，管理后台和代理端静态资源通过 CDN 发布，小程序仅访问已备案且配置到微信后台的 HTTPS 域名。腾讯云上的具体购买、安全组、Nginx、systemd、COS 与小程序域名步骤见 [腾讯云部署方案](腾讯云部署方案.md)。

环境至少区分 development、staging、production，每个环境使用独立数据库实例与凭据。生产环境禁止启用 Mock 支付成功入口；Provider 密钥、数据库凭据、加密密钥、CA 证书和对象存储凭据全部由 Secret 管理，不写入仓库。上线前确认 TencentDB 自动备份与 PITR（或等价日志备份），并完成 staging 恢复演练。生产地域必须在真实微信用户网络下完成延迟测试，并经隐私与数据驻留评审后确定；评审完成前不得写入真实手机号、地址或银行卡数据。

Development 前向迁移只允许在受控环境用 `scripts/db/deploy-development.mjs` 执行。操作者必须提交当前 main 的精确 `target_sha`，并设置 `DATABASE_MIGRATION_CONFIRM=DEVELOPMENT_MIGRATION_APPROVED`；脚本校验 TLS、TencentDB 主机、角色与迁移前历史后，才使用 `mall_migrator` 的 `DIRECT_URL` 执行 deploy。禁止并发迁移；代码回退仍不回滚加法索引。
...
CH-026 的 `0005` 沿用同一 main-only 受控 Workflow、确认短语和数据库 concurrency group。部署脚本只接受精确完成的 `0001→0004` 前置历史或已经完成的幂等 `0001→0005` 目标；部署前检查退款 attempt 重复事实，迁移本身继续检查来源包络与金额。代码回退时保留 0005 的加法防护，不执行数据库降级。
...
- 第三方物流与自动确认：在后续变更中增加 Provider/回调、签名验证、对账与可配置时钟；CH-024 只接受总部人工事件，不能以 Worker 推测 DELIVERED 或自动完成。
...
- CH-022 本地契约解析实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 705 schema refs / 2,678 local refs / 0 dangling refs；CH-023 不改变在线契约或统计，0004 继续作为已部署的最小权限函数修复保留，B10.5/B10.6 未新增迁移。B10 最终 SHA `f5e59169b53a97704711c3aae3049e5b5d16a930` 的[普通 CI Run 33305811318](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33305811318)与[Supabase development rollback-only smoke Run 33306877575](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33306877575)同 SHA 且均成功，B10 development `GO`，CH-021 已自动失效；真实支付、staging 和 production 仍为 `NO-GO`。
- CH-024 专属检查、生成与 Redocly lint 已通过，实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 705 schema refs / 2,695 local refs / 0 dangling refs。B11.1 已完成 FulfillmentRepository Admin/Store 查询投影、Admin 订单列表/详情/受控地址、Store 物流投影、共享 `display_status` 与 PII 审计；B11.2 已完成唯一包裹、精确剩余可发项、活动售后阻断、人工状态单向推进和纠错事实只追加；B11.3 已完成消费者/Admin 共享完成事务、规则/售后期限冻结、历史部分退款后剩余佣金 4.50 精确结转、钱包 CAS、审计、Outbox 与幂等原子收敛；B11.4 已完成 MP/ADM 履约工程交互、严格解码、资源绑定和五视口异常矩阵；B11.5 已完成真实纵向、全仓门禁、数据库/Redis/对象零残留复验和最终远端同 SHA 双绿。普通退款生产路径明确排除，正佣金钱包缺失或不一致时 fail-closed。B11 development `GO`，CH-025 已自动失效。
- CH-026 契约实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 706 schema refs / 2,726 local refs / 0 dangling refs，Redocly 0 warning。`0005` 本地完整五迁移回放、22 个 partial indexes、16 个自有函数、runtime 合法退款项写入、非法包络/金额拒绝、父 refund 并发串行和 `migration diff=0` 已通过；B12.0 远端三项证据也已闭合。B12.1-B12.6 本地实现保持 OpenAPI、Prisma 与既有迁移冻结；完整 `RETURN_REFUND` 真实纵向和精确清理已通过。最终实现 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿，B12 development `GO`，CH-027 已自动失效。B13.0-B13.8 的历史分批退出事实继续有效；B13 最终 SHA `250cbb824ad11f4b0e11338c19494655ec384ff6` 的 development migration `33952031401` 与随后 rollback-only smoke `33952283771` 同 SHA、依次成功，B13 development `GO`，CH-029 已失效。B14.0-B14.3 已完成并取得 development `GO`；当前基线为 `v2.4.13 / CH-032`、OpenAPI `2.4.13-ch032`。B17 仅增加脱敏 staging 环境门禁和 readiness，不改变业务契约或迁移链；既有 P2、B12 orphan 现场回收、恢复演练和外部复核继续阻断 staging、production、真实数据和真实资金。
