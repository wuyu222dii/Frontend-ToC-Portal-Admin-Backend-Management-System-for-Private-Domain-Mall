...
> 技术基线：产品/API 为 v2.4.10 / CH-026；数据库修复 CH-023（2026-08-30）继续有效。状态：B0-B11 development 已完成并维持 `GO`；B12.0 治理、契约与 `0005_b12_aftersale_refund_guards` 本地实施及 P0/P1 复核已完成，精确 SHA 远端三门禁和 B12.1 reviewer 条件尚未闭合。真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO`
...
- B12.0 只准入治理、契约、迁移和门禁。精确 SHA 必须依次通过普通 CI、Supabase development migration 和 rollback-only smoke；CH-027 尚未批准，B12.1 前仍需第二 reviewer 或另行批准的例外。
...
- CH-026 契约实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 706 schema refs / 2,720 local refs / 0 dangling refs，Redocly 0 warning。`0005` 本地完整五迁移回放、22 个 partial indexes、16 个自有函数、runtime 合法退款项写入、非法包络/金额拒绝、父 refund 并发串行和 `migration diff=0` 已通过；远端三项证据尚未取得，因此 B12.1 仍未准入。
