...
> 技术基线：v2.4.4 / CH-014（2026-08-25）；状态：B0-B5 development 已完成，B6.1 Store 公开 API 已完成本地及受控 Supabase development rollback-only 验收；B6.2 MP-01 至 MP-05 工程页面、API 客户端和 SKU 选择层已完成本地验收并暂停，B6.3/B6.4 尚未开始，staging/production 未批准
...
- B6.2 已通过跨端 Store API 客户端复用这 5 个公开 GET，并完成 MP-01 首页、MP-02 分类、MP-03 搜索、MP-04 商品详情和 MP-05 SKU 选择层。游客购物车仍是 B6.3 的独立边界，B6.2 不写本地购物车，也不伪造登录态或业务成功。
- B6.2 本地验收为 miniapp unit `4 files / 58 passed`、H5 与 MP-Weixin build 均通过、B6 UI Playwright `19 passed / 36 designed skips`；MP-01 至 MP-05 成功路径和无横向溢出覆盖 375/390/414/1024/1440 五个视口，当前在 B6.2 暂停点停下。
- B6.4 尚未执行真实 browser -> Nest -> PostgreSQL/MinIO 纵向链路，也未取得普通 CI 与 Supabase workflow 同一最终 SHA 双绿，不得把当前结果描述为 B6 最终 development `GO`。
...
- CH-014 实测为 172 paths / 196 operations / 196 unique operationId / 312 schemas / 691 schema refs / 2,577 local refs / 0 dangling refs；Store SKU/Product 售罄布尔投影、COMPREHENSIVE/HOT/NEWEST/价格排序、商品名搜索、无分页公开品牌/分类、首页四区状态与 HMAC IP 限流已纳入 B6.0 契约门禁。B6.1 公开 API 已按该契约实现；B6.2 MP-01 至 MP-05 已完成本地验收后暂停，下一入口为 B6.3 游客本地购物车。
- Prisma/schema/`0001_initial` 逐字节不变，Prisma validate 与 migration diff 为零。
- B5 实现基准 SHA `d97c43958142eaa0fa5a0a9954bb21d136944ba2` 的普通 CI Run `32822780209` 与 Supabase development rollback-only Run `32823898006` 为同一 SHA 双绿。CH-013 只接续 B6 脱敏 development 的单人维护补偿控制，B6.4 后自动失效，不放行 staging/production。
