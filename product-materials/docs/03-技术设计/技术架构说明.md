...
> 技术基线：v2.4.3 / CH-012（2026-08-25）；状态：B5.1 Banner API、B5.2 Inventory API 与 B5.3 ADM-07/08 后台工程已分别验收暂停，B5.4 尚未开始，staging/production 未批准
...
- B4 实现基准 SHA `0929f2435e7f5b9ad745fd9cab60b066378e502e` 的普通 CI Run `32721588213` 与 Supabase development rollback-only Run `32722510890` 均成功。本地/临时 PostgreSQL 与 MinIO 证据未被用于替代远端记录。CH-011 仅接续 B5 development 的单人维护补偿控制；B5.2 受控 Supabase rollback-only 只证明该批事务可回滚，不替代 B5.4 的准确 SHA 普通 CI 与 Supabase workflow 双绿，也不放行 staging/production。
