...
- CH-022 将关闭扩展为三段 Provider 流程：首个短事务 claim `CREATING/OPEN` 为 `CLOSE_PENDING`，事务外按稳定 `intent_no` query/close，明确不可支付后再在最终事务关单释放；未知结果继续保持占用。B10.1 当前只实现 Provider、支付意图和 Mock Inbox；三段关单属于 B10.3，尚未开始。
...
- `0003_b10_payment_fact_indexes` 只增加“每 intent 最多一个成功 attempt”和“每订单最多一个迟到退款”两个条件唯一索引；迁移前检测历史重复事实，既有迁移逐字节不变。B10.0 与 B10.1 本地范围已完成，B10.1 退出复审为 `P0=0/P1=0`；现暂停待批 B10.2，B10.2 未开始，B10 尚未 `GO`，CH-021 继续有效。
...
- CH-022 本地契约解析实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 705 schema refs / 2,678 local refs / 0 dangling refs；本地契约、迁移回放和权限门禁通过。order_id、payment_intent_id、refund_id 路径参数统一收紧为 ULID。B10.0 三项远端证据已在准确 SHA 闭合；B10.1 本地实现与验收已完成并以 `P0=0/P1=0` 退出，现暂停待批 B10.2。B10.2 未开始，B10 尚未 `GO`，CH-021 继续有效。
