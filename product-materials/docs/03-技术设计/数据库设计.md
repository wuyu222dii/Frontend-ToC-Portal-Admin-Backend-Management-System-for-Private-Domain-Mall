...
| 状态 | B5 已完成并取得普通 CI/Supabase rollback-only 同 SHA 双绿；CH-014 未修改逻辑模型或首迁移，B6.1 Store 只读 repository 已完成隔离 PostgreSQL full 与受控 Supabase development rollback-only 验收并暂停；B6.2 未开始，staging/production 未批准 |
| 更新时间 | 2026-08-25 |
...
- 这些数据库证据只关闭 B6.1 repository 门禁；B6.4 的最终 SHA workflow、真实 H5 纵向和同 SHA 双绿未开始，不得据此标记 B6 development `GO`。
