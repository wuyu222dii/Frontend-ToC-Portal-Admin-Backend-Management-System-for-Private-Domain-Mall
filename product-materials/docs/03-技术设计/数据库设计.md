...
| 状态 | B0-B8 development 已完成；B8 最终 SHA 已同 SHA 双绿，CH-017 已失效；CH-019/CH-020 已批准，当前仅进入 B9.0 前向索引迁移准入，B9.1 业务代码尚未开始；staging/production 未批准 |
| 更新时间 | 2026-08-28 |
...
CH-020 数据库结论：现有 76 models / 59 enums 已具备 B9 报价、订单、地址/归因快照、预占、流水、审计、Outbox 与幂等所需字段，不增表、列或枚举。两份 `0001_initial` 继续逐字节不变；`0002_b9_inventory_fact_indexes` 是只增加查询与事实唯一性的前向 migration，代码回退时保留不降级。受控 development 部署必须先在精确 main SHA 的普通 CI 绿色后执行 migration workflow，再运行共用 concurrency group 的 rollback-only smoke；未取得这两项远端证据前不得开始 B9.1。
