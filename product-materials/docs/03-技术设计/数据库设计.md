...
|---|---|
| 文档版本 | v2.4.4 |
| 对应基线 | MVP/PRD v2.4.4、CH-001 至 CH-014 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B5 已完成并取得普通 CI/Supabase rollback-only 同 SHA 双绿；CH-014 未修改逻辑模型或首迁移，B6.1 Store 只读 repository 已完成隔离 PostgreSQL full 与受控 Supabase development rollback-only 验收；B6.2 MP-01 至 MP-05 已完成本地验收且没有数据库变更，B6.3/B6.4 未开始，staging/production 未批准 |
| 更新时间 | 2026-08-25 |
...
- B6.2 只新增跨端 Store API 客户端、MP-01 至 MP-05 页面和 SKU 选择层，不写数据库，也没有实现 B6.3 游客本地购物车。其本地验收为 miniapp unit `4 files / 58 passed`、H5 与 MP-Weixin build 均通过、B6 UI Playwright `19 passed / 36 designed skips`；MP-01 至 MP-05 成功路径和无横向溢出覆盖 375/390/414/1024/1440 五个视口。
- 这些数据库证据只关闭 B6.1 repository 门禁；B6.4 的最终 SHA workflow、真实 H5 纵向以及普通 CI 与 Supabase workflow 同一最终 SHA 双绿尚未执行，不得据此标记 B6 development `GO`。
