...
| 状态 | B5 已完成并取得普通 CI/Supabase rollback-only 同 SHA 双绿；CH-014 未修改逻辑模型或首迁移，B6.1 Store 只读 repository 已完成隔离 PostgreSQL full 与受控 Supabase development rollback-only 验收；B6.2 MP-01 至 MP-05 与 B6.3 本地游客购物车均已完成本地验收且没有数据库变更，B6.4 验收入口已接入，普通 CI、Supabase rollback-only 与真实纵向证据待同一最终 SHA 执行，B6 仍为 `NO-GO`，staging/production 未批准 |
| 更新时间 | 2026-08-26 |
...
- 这些数据库证据只关闭 B6.1 repository 门禁。B6.4 验收入口已接入；最终 SHA workflow、真实 H5 纵向以及普通 CI 与 Supabase rollback-only 同一最终 SHA 双绿证据待执行，不得据此标记 B6 development `GO`。
