...
|---|---|
| 文档版本 | v2.4.1 |
| 对应基线 | MVP/PRD v2.4.1、CH-001 至 CH-006 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | CH-006 不修改逻辑模型或首迁移；B3.0 空库回放、权限、冻结核对与 migration diff 已通过，不代表 B3 业务实现或生产上线 |
| 更新时间 | 2026-08-13 |
...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`migrations/0001_initial/migration.sql` 由 Prisma 7.9.1 基线 diff 生成后追加 PostgreSQL 专属约束，已在一次性 PostgreSQL 18.3 空库完整回放：76 张表、17 个条件唯一索引、165 个 `CHECK`、76 张表启用 RLS 且建立 76 个 runtime policy，共 24 个用户触发器（对应 45 个事件绑定）。`mall_migrator` 拥有 76 张应用表、59 个枚举和 15 个迁移函数。该结果是设计准入证据，不替代 staging 回放，且不得直接连接生产库验证。
...
| `product` | SPU 商品 | `spu_code` 全局唯一；必须属于一个有效品牌和一级分类，价格不在 SPU 保存 |
| `sku` | 成交、库存和佣金最小单位 | SKU code 唯一；仅保存零售价和推荐状态，不存在会员价；产生订单或库存流水后不可改码 |
...
| `product_image` | 商品与 READY 素材的有序关系 | 软删除；活动 `(product_id,sort_order)` 与 `(product_id,file_id)` 分别唯一，发布至少一张活动主图 |
| `banner` | 首页 Banner | 有效期、启停、排序和跳转目标 |
| `favorite` | 消费者收藏 | customer + product 唯一 |
| `cart` | 登录客户购物车 | `customer_id` 普通唯一；游客购物车仅在小程序本地，不建匿名行 |
...
2. `payment_attempt.status = SUCCEEDED_LATE`，真实记录已收款；原 `payment_intent` 保持 `CLOSED/EXPIRED`，不得改回 OPEN；
...
所有伪代码使用同一锁序：请求守卫 → 账户/客户/代理 → 订单 → 订单项 → 支付/退款/售后/包裹 → SKU/库存 → 佣金 → 钱包 → 只追加审计/Outbox。同级按稳定 ID 升序；Provider 调用永远在提交之后执行，且回写使用新的短事务。
...
BEGIN
  ...
  锁定 customer_profile，校验并递增 version，再读取当前绑定
  创建 sales_order 聚合根及 order_item 快照
  sales_order.created_at/pay_expires_at 使用数据库默认，CHECK 固定二者相差 30 分钟
  按 sku_id 升序锁定 SKU、inventory_balance
  服务端重新计价
  创建 address snapshot、attribution candidate
...
事务外以 intent_no 作为商户支付号调用 Provider create；网络错误时立即 query(intent_no)，不得创建第二商户号

BEGIN
  按全局锁序重新锁定 order -> payment_intent
  Provider 明确存在则写 provider_intent_id/provider_state，状态 -> OPEN
  仍不确定则保留 CREATING 并推进 next_reconcile_at
  写审计/Outbox
COMMIT
```

Provider 调用失败、进程在外部成功后崩溃或本地回写冲突都不创建第二笔意图。Worker 扫描 `CREATING/OPEN/CLOSE_PENDING`，使用 `query(intent_no)` 恢复真实状态；关闭失败或未知继续保持 `CLOSE_PENDING`，只写 `last_error_code/next_reconcile_at`。只有原意图已明确 `CLOSED/FAILED/CANCELLED/EXPIRED` 且订单未过期，才允许创建新 `intent_no`。
...
BEGIN
  ...
  按 sku_id 锁定 inventory_balance/reservation，再锁当前 rule version
  ...
  若为 DIRECT，或 commission_base/effective_rate/original_commission 任一为 0：position -> NONE，金额全为 0，跳过佣金/钱包流水与佣金 Outbox
...
代理停用事务也必须先锁同一 `agent_profile`，再处理其邀请码、候选和会话；支付和停用谁先提交，谁决定本次归因，后提交者在锁内看到新 version 后按最新事实裁决。整单必须使用同一个 `commission_rule_version_id`。规则解析失败时先持久化支付事实，将订单置于财务/结算异常待办，再由幂等补偿任务重试；禁止使用客户端比例或写入不完整快照。

### 6.4 主动取消、超时关闭与迟到回调

用户主动取消和超时任务复用同一三段服务，避免持锁调用网络；差异仅为最终 `close_reason`：

```text
BEGIN
  claim idempotency_record
  锁 order -> 全部 order_item -> payment_intent
  无意图：直接进入最终关闭分支
  CREATING/OPEN：状态 -> CLOSE_PENDING，记录 close_requested_at 后提交
  CLOSE_PENDING：复用同一 intent_no，不重复生成支付意图
COMMIT

事务外 query(intent_no)，必要时 close(intent_no)

BEGIN
  按全局锁序重新锁 order -> 全部 order_item -> payment_intent -> inventory reservation/balance
  Provider 明确 CLOSED/NOT_FOUND/EXPIRED/CANCELLED/FAILED：intent -> 对应终态，order -> CLOSED/USER_CANCELLED 或 PAYMENT_TIMEOUT，释放库存
  Provider 已成功：交给正常成功或迟到成功处理，不释放库存两次
  结果不确定/关闭调用失败：intent 保持 CLOSE_PENDING，更新 last_error_code/next_reconcile_at，订单和库存保持不变
COMMIT
```

若关闭与成功回调竞争，两边都按 order → 全部 order_item → payment_intent 的顺序复核 Provider 事实，只能产生正常支付或关闭后迟到退款之一。无支付意图时仍必须经过最终事务并按 order → 全部 order_item → inventory 顺序释放，不允许使用反向锁序的“快速分支”。
...
- Product create/update 的 `images[{file_id,sort_order}]` 表示活动图集原子替换。事务先锁商品和现有图关系，校验文件 READY、用途/归属与排序唯一，再软删除被移除关系并插入新关系。发布/启用商品前必须存在至少一张活动图，最小 sort_order 为主图；重放由请求幂等键收敛，不能留下半套图集。
...
- CH-006 不新增 `completed_at` 或其他数据库字段。`file_asset.status=READY`、`updated_at`、最终对象 key、实测 hash/MIME/size、审计和 `idempotency_record` 联合表达完成事实；不能把 `updated_at` 单独解释为只读完成时间。
...
- complete 同键同请求由 B3.1 的闭合 `FILE_UPLOAD_COMPLETE` 策略精确重放；不同键对 READY 再次 complete 返回 409。上传 intent、download URL 和 lifecycle preview 只用 `HASH_ONLY`，`response_body` 不保存签名 URL、请求头或 preview token。
...
- 商品：`product(status, category_id, brand_id, published_at)`、`sku(product_id, status)`；PRICE 排序取当前可见 ACTIVE SKU 最低 `retail_price`，同价按 product ID；HOT 销量为支付数量减成功退款数量的全生命周期净销量，`product.sales_count` 只是可重建缓存并由支付/退款事务投影和每日对账维护；
...
本项目不启用 Prisma `partialIndexes` preview；条件唯一、跨字段校验、RLS 和角色权限统一在 PostgreSQL SQL migration 中管理。首版 migration 共有 17 个条件唯一索引，直接建立在事实列和谓词上，不引入辅助字段：
...
`order_item_commission_snapshot`、`commission_ledger`、`inventory_ledger` 和 `audit_log` 等不可变事实表，对 `mall_runtime` 仅授予所需的 `SELECT`/`INSERT`，撤销 `UPDATE`/`DELETE`。运行角色全局撤销 DELETE，只对账号删除流程中没有历史保留价值且 API 明确硬删除的 `favorite`、`cart_item`、`customer_phone_verification` 与 `customer_address` 四表例外授权；其他生命周期删除均走 UPDATE 软删除。四表删除只能由满足资格检查、锁定账户且写审计的账号删除事务调用，普通 CRUD 不因此获得跨客户删除能力。`callback_inbox` 的 Provider event ID、验签结果和原始 payload 不可修改，只允许更新处理状态、重试计数、处理时间和脱敏错误字段，可用 PostgreSQL 列级 `UPDATE` 授权或受控存储过程实现。佣金原始快照和原正向流水不得因退款被覆盖，退款只能追加 `EXPECTED_REDUCED`、`EXPECTED_CANCELLED` 或 `REFUND_DEBIT`。

## 8. 加密、脱敏与删除

- 账户手机号、地址电话、银行卡号和代理联系电话使用字段级信封加密；需要精确去重/检索的字段另存 HMAC hash，展示另存 last4。代理联系电话当前不做精确检索，因此不保存明文或确定性密文，只保存随机密文、last4 和 key id。
...
- 品牌名称、分类名称、SPU code 和 SKU code 采用“软删除后仍全局保留、永不复用”口径。恢复必须更新原记录并校验上游依赖、`If-Match` 和状态；再次创建相同业务键返回 `SOFT_DELETED_KEY_RESERVED`，不得通过新 ID 绕过历史引用。恢复后若存在名称/展示冲突返回 409，并由管理员先处理冲突，不自动改名。
...
- 品牌/分类创建只能为 DRAFT，`sort_order >= 0` 且列表按 `sort_order,id` 稳定；默认查询排除 deleted_at 非空记录，显式 ARCHIVED 查询和详情允许恢复。Product/SKU 生命周期不接受 ACTIVATE。
...
CH-006 数据库结论：现有 76 models / 59 enums 已具备品牌、分类、文件和幂等所需字段，本轮未创建迁移。根目录与本目录的 Prisma schema、`0001_initial` 已逐字节核对保持原样；`prisma validate`、PostgreSQL 18.3 空库回放、权限故障注入、冻结校验和 migration diff=0 均已通过。
