...
|---|---|
| 文档版本 | v2.4.9 |
| 对应基线 | MVP/PRD v2.4.9、CH-001 至 CH-024；产品/API 以 CH-024 为准 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B0-B10 development 已完成并维持 `GO`；B11.0-B11.3 本地工程门禁已通过，B11.3 复审 `P0=0/P1=0/P2=2` 并暂停在 B11.4 前；B11 总体不得 `GO`。CH-025 已批准且仅适用于 B11.1-B11.5 脱敏 development。B11.1-B11.3 均未修改 Prisma 或 migration，两份 Prisma schema 与 `0001` 至 `0004` 继续逐字节不变；真实客户数据、staging、production、真实支付与真实物流均为 `NO-GO` |
| 更新时间 | 2026-08-30 |
...
- 多包裹、第三方物流 Provider、自动确认收货和普通售后属于后续独立变更；CH-024 不预建数据库结构。B11.1-B11.3 已复用既有模型实现 Fulfillment 查询、唯一包裹发货、追加式人工物流、订单完成与佣金一次结转，使用 Serializable 锁内重验，没有新增或修改数据库结构。普通退款生产路径未纳入；本批只验证活动退款阻断和历史部分退款后剩余佣金 4.50 结转。正佣金代理钱包必须由上游预创建，缺失或不一致时完成事务 fail-closed 并整笔回滚。
...
CH-024 数据库结论：OpenAPI 专属检查、生成与 Redocly lint 已通过，实测为 173 paths / 198 operations / 198 unique operationId / 326 schemas / 705 schema refs / 2,695 local refs / 0 dangling refs；在线契约不要求数据库结构变化。现有 76 models / 59 enums 与完整 `0001→0004` 链足以承载唯一单包裹、人工物流、消费者/管理员完成、业务规则冻结和佣金一次结转，本轮数据库变更数固定为 0。B11.0 已验证两份 Prisma schema 与四份既有 migration 逐字节不变、Prisma schema 有效且 remote `migration diff=0`；B11.3 继续保持 Prisma/migration 零改动，数据库全量为 **32 files passed / 16 skipped；611 passed / 96 skipped**。隔离 PostgreSQL full **10/10 passed**，覆盖 CUSTOMER/Admin 竞争与 SHIPPED/IN_TRANSIT/DELIVERED 三种包裹状态；Supabase development rollback-only **9 passed / 1 full-only skipped**。full 集成 cleanup SQL 完整，但自动无残留断言只覆盖核心 fixture；全面枚举留在 B11.5。完整回放、权限、原生指纹与最终远端同 SHA 双绿仍在 B11.5 复验。普通退款生产路径排除，本批只验证活动退款阻断和历史部分退款后剩余佣金 4.50 精确结转；正佣金钱包缺失或不一致时 fail-closed，钱包预创建为上游前置条件。B11.3 本地工程门禁已完成，复审 `P0=0/P1=0/P2=2` 并暂停在 B11.4 前；B11.3 的 P2 为未来普通全额退款改变完成原因后原 HASH_ONLY 键重放 fail-closed 500，B11.2 的既有 P2 仍只记录 CH-024 未冻结相邻 STATUS 事件时间单调性。B11 总体不得 `GO`，多包裹、第三方物流、自动确认与普通售后继续 `NO-GO`。
