...
|---|---|
| 文档版本 | v2.4.5 |
| 对应基线 | MVP/PRD v2.4.5、CH-001 至 CH-016 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B0-B6 development 已完成；CH-015/CH-016 已批准，B7.0-B7.3 已完成并按批暂停，B7.3 `P0=0/P1=0`，B7.4 未准入；CH-016 不修改逻辑模型或首迁移，staging/production 未批准 |
| 更新时间 | 2026-08-27 |
...
- Product/SKU lifecycle confirm 的唯一锁序为 `idempotency/high_risk_preview -> brand -> category -> product -> sku -> inventory_reservation -> audit/outbox`，同类按 ID 升序。事务内重新读取所有状态和版本，不以 preview 时快照代替最终校验。
...
- 库存人工调整按 `idempotency/high_risk_preview -> sku -> inventory_balance -> active reservation IDs -> inventory_ledger -> audit/outbox` 锁序。成功事务以条件更新 physical 和 version，插入恰好一条 `MANUAL_INCREASE/MANUAL_DECREASE` 流水并消费 preview；失败整体回滚且不消费。
...
- 注销 preview 复用现有 high-risk preview/hash/idempotency 能力并绑定 account version；只有 eligible 才签发 5 分钟 token。同步 confirm 重检 blocker facts，在同一事务更新 account/customer profile、撤销所有 session/identity、清理 phone/candidate/binding/允许硬删个人数据、匿名化 privacy projection 并写 audit/idempotency/outbox；失败整体回滚。
- 注销 confirm 为 HASH_ONLY，不保存完成响应。现有 account、external identity、session、consent、customer profile/phone、candidate/binding、account deletion request、privacy projection、audit/idempotency/outbox 已足够承载 CH-016，结构变更数为 0。
...
CH-016 数据库结论：现有 76 models / 59 enums 足以承载 B7 身份、会话、同意、资料手机号、归因绑定、隐私投影和同步注销，前提是 deployment 固定单一消费者微信 AppID，且 `wechat_union_id` 不参与登录或自动合并；迁移数固定为 0。两份 schema/首迁移逐字节校验、Prisma validate、全新 PostgreSQL 回放、权限故障注入和 migration diff=`No difference detected` 已全部通过；B7.1 身份会话、B7.2 profile/手机号和 B7.3 归因/服务代理 repository/service 的 full/rollback 门禁均已通过且无 fixture 残留。未来多 AppID 必须另立变更和迁移，B7.4 仍须另行批准。
