...
|---|---|
| 文档版本 | v2.4.8 |
| 对应基线 | MVP/PRD v2.4.8、CH-001 至 CH-022 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B0-B9 development 已完成并维持 `GO`；B10.0 已在 SHA `8cd5781eed9349d6f110fa43510e78c7f525a482` 取得普通 CI Run `33242561514`、Supabase development migration Run `33243979003`、rollback-only smoke Run `33244107293` 三项成功证据。B10.1 Provider/支付意图/Mock Inbox 本地实现与验收已完成并以 `P0=0/P1=0` 退出，现暂停等待用户批准进入 B10.2；B10.2 未开始，B10 尚未 `GO`，CH-021 继续有效。staging、production 与真实支付均为 `NO-GO` |
| 更新时间 | 2026-08-29 |
...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`0001_initial` 与 `0002_b9_inventory_fact_indexes` 继续逐字节冻结；CH-022 只在其后增加 `0003_b10_payment_fact_indexes`，不增加表、列或枚举。完整链保持 76 张表、165 个 `CHECK`、76 张表启用 RLS、76 个 runtime policy、24 个用户触发器（45 个事件绑定）；0003 额外增加两个支付事实条件唯一索引。`mall_migrator` 的既有对象所有权模型不变。本地空库/升级回放、权限、冻结指纹与 migration diff 已通过，development 远端部署和 rollback-only smoke 已在 B10.0 准确 SHA 成功闭合。
...
本项目不启用 Prisma `partialIndexes` preview；条件唯一、跨字段校验、RLS 和角色权限统一在 PostgreSQL SQL migration 中管理。冻结的 `0001_initial` 共有 17 个条件唯一索引；CH-020 的 `0002_b9_inventory_fact_indexes` 增加第 18 个，CH-022 的 `0003_b10_payment_fact_indexes` 再增加两个支付事实条件唯一索引。两次迁移都先做历史重复预检，不引入辅助字段：
...
CH-022 数据库结论：现有 76 models / 59 enums 已具备 B10 payment intent/attempt、Inbox、reservation、库存流水、归因、佣金、refund、审计、Outbox 和 reconciliation task 所需字段，不增表、列或枚举。`0003_b10_payment_fact_indexes` 只增加 intent 成功事实与订单迟到退款两个条件唯一索引；本地完整迁移回放、历史重复预检、权限、冻结指纹和 `migration diff=0` 已通过。B10.0 已在准确 SHA 取得普通 CI、Supabase development migration、rollback-only smoke 三项成功证据。B10.1 仅写现有 payment intent 与 Callback Inbox 事实，Mock Redis 状态保留 7 天，初始 `next_reconcile_at` 为数据库当前时间后 60 秒，Provider 调用保持在数据库事务外；既有 `CREATING` 先 query、仅 `NOT_FOUND` 才 create，`OPEN` 与订单 `PROCESSING/version+1` 原子提交，明确终态与订单 `UNPAID` 原子恢复，陈旧 `If-Match` 新命令返回 409。B10.1 本地实现与验收已完成并以 `P0=0/P1=0` 退出，现暂停待批 B10.2；B10.2 未开始，B10 尚未 `GO`，CH-021 继续有效。
