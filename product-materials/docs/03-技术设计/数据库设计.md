...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`0001_initial` 至 `0006_b13_agent_finance_guards` 继续逐字节冻结；CH-032 在其后增加前向 `0007_b15_development_convergence_guards`，只增加佣金目标名快照列、约束、READY 完成事件唯一索引及关联竞态守卫，不增加表或枚举。B13.0 的既有完整链实测证据保持；`0007` 尚未部署到 development，部署后只前向保留。
