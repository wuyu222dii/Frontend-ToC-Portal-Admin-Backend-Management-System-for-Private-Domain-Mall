...
|---|---|
| 文档版本 | v2.4.2 |
| 对应基线 | MVP/PRD v2.4.2、CH-001 至 CH-010 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | CH-010 未修改逻辑模型或首迁移；B4.0 至 B4.4 development 已完成，Prisma/首迁移冻结门禁与 Supabase rollback-only 均通过，不代表 staging/production 上线批准 |
| 更新时间 | 2026-08-25 |
...
CH-010 数据库结论：现有 76 models / 59 enums 已具备 Product、Sku、ProductImage、InventoryBalance、预览、幂等和审计所需字段，本轮迁移数固定为 0。根目录与本目录的 Prisma schema、`0001_initial` 必须逐字节保持原样；B4.0 只有在 `prisma validate`、冻结校验、空库回放、权限检查和 migration diff=0 全部通过后才可验收。
