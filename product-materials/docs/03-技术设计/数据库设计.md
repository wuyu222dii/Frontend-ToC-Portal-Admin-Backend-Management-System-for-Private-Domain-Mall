...
|---|---|
| 文档版本 | v2.4.10 |
| 对应基线 | MVP/PRD v2.4.10、CH-001 至 CH-026；产品/API 以 CH-026 为准 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B0-B11 development 已完成并维持 `GO`；CH-025 已自动失效。CH-026/CH-027 已批准，B12.0 精确 SHA 三项远端门禁已闭合；B12.1-B12.5 已完成并在 B12.5 验收后暂停。两份 Prisma schema 与既有 `0001` 至 `0005` 继续逐字节不变，B12.5 未新增迁移；B12.6 尚未开始，B12 仍非 development `GO`，CH-027 仍有效。真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO` |
| 更新时间 | 2026-08-30 |
...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`0001_initial` 至 `0004_b10_commission_position_trigger_fix` 继续逐字节冻结；CH-026 在其后增加事务型 `0005_b12_aftersale_refund_guards`，仍不增加表、列或枚举。完整链保持 76 张表、165 个 `CHECK`、76 张表启用 RLS、76 个 runtime policy，实测为 22 个 partial indexes、26 个用户触发器（50 个事件绑定）和 16 个自有/runtime 函数；远端精确 SHA 证据已由 B12.0 门禁闭合，B12.1 未新增 schema 或 migration 变更。
