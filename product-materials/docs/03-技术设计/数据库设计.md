...
| 状态 | B0-B12 development 已完成并维持 `GO`；CH-025、CH-027 已自动失效。CH-028/CH-029 已批准，B13.0 本地候选已通过回放、权限、冻结、diff 与零残留门禁；最终实现 SHA 的远端普通 CI -> development migration -> rollback-only smoke 尚未执行，B13.0 未退出、B13.1 尚未准入。B12 最终 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿。B12 orphan `P2=1` 继续阻断 staging/真实数据，真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO` |
| 更新时间 | 2026-09-02 |
...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`0001_initial` 至 `0005_b12_aftersale_refund_guards` 继续逐字节冻结；CH-028 在其后增加前向 `0006_b13_agent_finance_guards`，仍不增加表、列或枚举。B13.0 本地完整链实测为 76 张表、175 个 `CHECK`、76 张 RLS 表、76 个 runtime policy、23 个 partial indexes、47 个用户触发器（92 个事件绑定）和 28 个自有函数；完整回放、权限故障、原生定义指纹、冻结文件、零残留及 `migration diff=0` 已通过。该结论尚无最终实现 SHA 的远端三门禁证据。
