...
| 状态 | B0-B11 development 已完成并维持 `GO`；CH-025 已自动失效。CH-026/CH-027 已批准，B12.0 精确 SHA 三项远端门禁已闭合；B12.1 复用现有模型完成最小 Store 售后后端并在 `P0=0/P1=0` 后暂停。两份 Prisma schema 与既有 `0001` 至 `0004` 继续逐字节不变，B12.1 未新增迁移；B12.2 尚未开始。真实客户数据、staging、production、真实微信退款与真实物流均为 `NO-GO` |
| 更新时间 | 2026-08-30 |
