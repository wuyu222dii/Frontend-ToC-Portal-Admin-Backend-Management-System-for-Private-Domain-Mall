...
|---|---|
| 文档版本 | v2.4.12 |
| 对应基线 | MVP/PRD v2.4.12、CH-001 至 CH-030；产品/API 以 CH-030 为准 |
| 数据库 | 腾讯云 TencentDB PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B13 development `GO`；`0006` 只前向保留。B14 不新增数据库迁移，也不读写已预埋的 `sales_daily_aggregate`；B14.1 仅在准入后以只读一致性快照查询权威事实。B12 orphan 与 B13 已登记 P2 继续阻断 staging、production、真实数据和真实资金。 |
| 更新时间 | 2026-09-05 |

## 1. 设计原则

- 使用一个腾讯云 TencentDB PostgreSQL 主库承载首期单商户、单仓、单层代理业务；按领域划分表，不按三端重复建表。
...
- 远程库只使用腾讯云 TencentDB PostgreSQL。进程拒绝任何 `SUPABASE_*` 变量和 `*.supabase.co` / `*.supabase.com` 主机。微信自建认证、会话和 RBAC 的唯一身份源仍是 `account` 等 `public` 应用表。
- 76 张应用表全部位于 `public`。三端不得持有数据库连接串或 `service_role` 密钥。
- NestJS API/Worker 是唯一数据访问入口，运行时只使用 `mall_runtime`；该角色仅获所需 DML、序列使用权及专用 RLS policy，无 DDL、schema ownership、角色管理或 `BYPASSRLS`。
- 首次安装由 TencentDB 实例管理员创建角色并回放基线 migration；迁移末尾将应用表、枚举和迁移函数的所有权移交给 `mall_migrator`。后续 migration 只使用 `mall_migrator`，运行时只使用 `mall_runtime`；凭据分别存入 Secret Manager 并独立轮换，禁止以实例管理员或 `service_role` 运行应用。
- 所有应用表启用 RLS。首迁移创建的无登录 `anon`、`authenticated` 不授予表/序列权限且不创建 policy；`service_role` 也撤销应用表权限。只为 `mall_runtime` 建立允许访问的连接角色 policy，具体客户、代理和管理员数据范围仍由 NestJS RBAC 校验。

| 场景 | 连接 | 约束 |
|---|---|---|
| 长驻 API/Worker | TencentDB 内网 | 使用内网域名；使用有界连接池，可使用 prepared statements |
| migration、`pg_dump` 与恢复 | TencentDB 内网 | 只使用 `mall_migrator` 的 `DIRECT_URL` |

所有远程连接必须 `sslmode=verify-full` 并使用腾讯云 CA。按 Prisma ORM 7 约定，`schema.prisma` 不声明 URL；`prisma.config.ts` 的 `datasource.url=env('DIRECT_URL')` 只供 CLI/migration，NestJS API/Worker 通过 `@prisma/adapter-pg` 读取 `DATABASE_URL`。密码、CA 和加密密钥不得进入仓库、日志或前端构建产物。
...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`0001_initial` 至 `0006_b13_agent_finance_guards` 继续逐字节冻结；CH-032 在其后增加前向 `0007_b15_development_convergence_guards`，只增加佣金目标名快照列、约束、READY 完成事件唯一索引及关联竞态守卫，不增加表或枚举。B13.0 的既有完整链实测证据保持；`0007` 已在 development 通过最终 SHA 迁移门禁部署，并按前向策略保留。
...
- 创建包裹按幂等事实 → 管理员 → order → 全部 order_item → 活动 aftersale → shipment 的锁序，并校验订单 `If-Match`。首期为单包裹，只允许 `PENDING_SHIPMENT/READY_TO_SHIP` 发货；只要订单存在 `PENDING_REVIEW/REFUNDING/WAITING_RETURN/WAITING_RECEIPT/RETURN_EXCEPTION/REFUNDING_AFTER_RETURN/REFUND_FAILED` 的活动售后，整单返回 `ACTIVE_AFTERSALE_BLOCKS_SHIPMENT`，不得发未占用行。
- `shipment.order_id` 普通唯一，订单整个生命周期最多一条包裹；创建事务必须同时写非空 carrier/tracking/shipped_at、全部 items 并直接将 shipment 置 `SHIPPED`、订单置 `SHIPPING/SHIPPED`，不存在 CREATED 中间记录。请求 items 必须与全部剩余可发订单项集合精确相等，每个订单项恰好一次，且 `shipment_item.quantity = quantity - pre_shipment_refunded_qty - shipped_qty`；缺失、重复、多出或数量不等返回 `SHIPMENT_ITEMS_MISMATCH`，事务零写入。未发货退款成功同时增加 `refunded_qty` 与 `pre_shipment_refunded_qty`；发货后的退货退款只增加 `refunded_qty`，因此全量发货后仍可合法部分/全额退款。确认弹窗取消时不落包裹；记录创建后不提供取消或替换端点，承运商/运单纠错只追加带原因的物流更正事件和审计，不能删除原事实。
- 物流节点只追加；客户端不提交 `event_key`，服务端从已鉴权的 HASH_ONLY 幂等事实派生稳定值，`(shipment_id,event_key)` 唯一约束仅作数据库并发兜底。应用事务还必须校验包裹 `If-Match`。STATUS 只允许 `SHIPPED -> IN_TRANSIT -> DELIVERED`，并同步订单履约轴；跳级、倒退或 DELIVERED 后改变状态返回 `SHIPMENT_STATE_CONFLICT`。TRACKING_CORRECTION 只追加完整新承运商/运单与原因，不改变状态。
...
订单完成必须写 `completion_reason`。消费者确认产生 `CUSTOMER_CONFIRMED`，管理员兜底为 `ADMIN_FORCED`；两者都只允许已支付/NORMAL、order=`SHIPPING` 且唯一 shipment 已处于 `SHIPPED|IN_TRANSIT|DELIVERED`、无活动售后或未解决支付异常的订单，事务先封存 shipment/fulfillment=`DELIVERED` 再完成，其他状态返回 `ORDER_NOT_RECEIVABLE`。该路径不补写跳过的人工物流事件；当前版本没有自动确认收货。发货后全额退款为 `COMPLETED/FULL_REFUND_AFTER_SHIPMENT`，不是 CLOSED。完成事务严格锁 idempotency → actor/customer → order → 全部 order_item → 活动 refund/aftersale → shipment → 当前 PUBLISHED business_rule_version → commission snapshot/position → agent_wallet，只把每个 `EXPECTED` position 当时的 `expected_remaining` 从预计结转为 `AVAILABLE_CREDIT`；`NONE` position 不写零金额流水，已经被退款减少的部分不得再次入账。订单完成、冻结当前 `business_rule_version_id`、计算 `aftersale_expires_at`、更新 position、追加唯一幂等佣金流水和钱包余额必须在一个本地数据库事务提交，Outbox 仅通知下游，不承担资金正确性。消费者与管理员并发完成必须由订单版本、行锁和佣金账本幂等键收敛为一次完成、一次结转。`agent_wallet.available_balance` 是缓存，事实以 `commission_ledger` 为准。
...
### 6.22 CH-024 单包裹履约零迁移结论

- 现有 `sales_order`、`order_item`、`shipment`、`shipment_item`、`logistics_event`、`business_rule_version`、佣金 snapshot/position/ledger、wallet、audit、outbox 与 idempotency 表已完整承载 B11，不新增表、列、枚举、索引、约束或触发器。
- `sales_order.version` 保护发货与完成，`shipment.version` 保护物流事件；`shipment(order_id)` 普通唯一与 `(shipment_id,event_key)` 唯一约束分别兜底唯一包裹和唯一人工事件。业务状态跳转、精确 items 集合和活动售后检查在 Serializable 事务的锁内重验，不能依赖事务外投影裁决。
- 四类履约写操作均为 HASH_ONLY，`idempotency_record.response_body` 必须为 null；只保存请求哈希、资源 ULID、状态码和响应摘要哈希。订单、包裹、物流、地址和佣金投影不得写入幂等响应缓存。
- 两份 Prisma schema 与 `0001_initial`、`0002_b9_inventory_fact_indexes`、`0003_b10_payment_fact_indexes`、`0004_b10_commission_position_trigger_fix` 逐字节不变。B11.0 必须以冻结哈希、Prisma validate、完整 `0001→0004` 回放、权限/原生指纹和 `migration diff=0` 证明零迁移；不能用“现有表已足够”代替机器证据。
- 多包裹、第三方物流 Provider、自动确认收货和普通售后属于后续独立变更；CH-024 不预建数据库结构。B11.1-B11.3 已复用既有模型实现 Fulfillment 查询、唯一包裹发货、追加式人工物流、订单完成与佣金一次结转；B11.4 只增加前端工程与契约实施勘误，继续没有新增或修改数据库结构。普通退款生产路径未纳入；本批只验证活动退款阻断和历史部分退款后剩余佣金 4.50 结转。正佣金代理钱包必须由上游预创建，缺失或不一致时完成事务 fail-closed 并整笔回滚。

### 6.23 CH-026 售后退款事实防护

- `0005_b12_aftersale_refund_guards` 是事务型前向迁移，不新增表、列或枚举；两份 Prisma schema 和既有 `0001` 至 `0004` 逐字节不变。正式与产品材料镜像 SQL 的 SHA-256 均为 `95f362667bdc6a0b751ae636d91a139a71a3f40155ba764937db01d5bbce412b`。
- 迁移以 5 秒 `lock_timeout` 获取 `sales_order/order_item/aftersale/aftersale_item/manual_compensation/refund/refund_item/refund_attempt` 的 `SHARE` 锁，再拒绝每个 refund 多个 `INITIATED|PROCESSING` 或多个 `SUCCEEDED` attempt、跨 customer/order/source/item 包络、金额补偿跨订单项及 refund 头金额与明细合计不一致的历史事实；任一异常使事务整体失败，不自动修复资金数据。远端 deploy 在 Prisma 前镜像这些已知数据预检，避免污染精确迁移历史。
- `enforce_refund_item_return_limit()` 保持 `SECURITY INVOKER`，移除读取不可变 `return_inspection_item` 时不必要的 `FOR SHARE`，并将父 refund 锁提升为 `FOR UPDATE`；`mall_runtime` 不获得验货事实 UPDATE/DELETE，合法 RETURN_REFUND 写入仍由父 refund、`aftersale_item` 锁和冻结验货结论约束。
- 新增 `uq_refund_attempt_one_active_per_refund` 和 `uq_refund_attempt_one_success_per_refund` 两个条件唯一索引；应用层的幂等、锁序和 Provider 收敛仍是第一道防线，索引用于阻断不同键并发的重复外部事实。
- 新增 `enforce_refund_envelope()` 及 refund/refund_item 两组 `DEFERRABLE INITIALLY DEFERRED` 约束触发器，允许同一事务先写退款头再写明细，并按去重升序父 refund ID 加锁后在提交时校验来源包络和 `refund.amount = SUM(refund_item.amount)`。MANUAL_COMPENSATION 必须精确一条 `quantity=1` 的资金归属明细且绑定 compensation 的 `order_item_id`；规则同时兼容 AFTERSALE 和 B10 LATE_PAYMENT。
- 本地完整回放实测为 22 个 partial indexes、26 个用户触发器、50 个事件绑定、16 个 `mall_migrator` 自有且 `mall_runtime` 可执行函数；函数、触发器和条件索引指纹分别为 `f50cb72196e16f0a5505eb53dcd461d1`、`5e1a4fbb1796c6b503e2ed77df474cc3`、`ad8fe12c0942686b13f55e244cfa2f27`。
...
- `refund_attempt(refund_id) WHERE status IN ('INITIATED','PROCESSING')` 与 `refund_attempt(refund_id) WHERE status='SUCCEEDED'`，分别保证每个退款最多一个活动尝试和一个成功尝试；
...
本项目不启用 Prisma `partialIndexes` preview；条件唯一、跨字段校验、RLS 和角色权限统一在 PostgreSQL SQL migration 中管理。冻结的 `0001_initial` 共有 17 个条件唯一索引；CH-020 的 `0002_b9_inventory_fact_indexes` 增加第 18 个，CH-022 的 `0003_b10_payment_fact_indexes` 再增加两个支付事实条件唯一索引，CH-023 的 0004 只替换触发器函数；CH-026 的 0005 再增加两个退款 attempt 条件唯一索引，实测总数为 22。所有索引迁移都先做历史重复预检，不引入辅助字段：
...
- 启用 TencentDB 自动备份和日志备份 / PITR，并在上线门禁中记录备份频率、保留期、恢复停机窗口和实际 RPO/RTO。
- 另行通过 TencentDB 内网定期执行加密的逻辑备份，例如 `pg_dump --format=custom --no-owner --no-acl`，备份文件保存到独立受控存储并验证校验和。
- 至少每季度在 staging 做一次完整恢复演练，核对 76 张应用表、部分唯一索引、`CHECK`、RLS、默认权限、触发器/函数及复算结果。托管备份不保存自定义角色密码时，须由基础设施脚本和 Secret Manager 在恢复后重新设置 runtime/migration 凭据。
...
- CH-024 数据库门禁必须覆盖：两份 Prisma schema 与 0001-0004 哈希逐字节不变；完整链仍恰有四条成功 migration history；现有 76 models / 59 enums、20 个 partial indexes、权限与原生定义指纹不漂移；最终 `migration diff=0`。履约集成测试必须证明并发唯一包裹、items 精确覆盖、活动售后阻断、物流状态单向、重复 event_key 收敛、两种完成竞争只成功一次、PUBLISHED 规则冻结及 EXPECTED 佣金只结转一次。
- CH-026 数据库门禁必须覆盖：既有 0001-0004 哈希不变、双份 0005 逐字节一致；空库完整回放和 0004→0005 升级；历史重复 attempt、跨主体包络、金额补偿跨订单项和金额不一致均原子失败；真实 `mall_runtime` 合法退货/迟到/金额补偿退款成功、跨订单项补偿失败、同 refund 两连接串行且不可变验货行仍不可写；两个索引、两个约束触发器、16 个函数的所有权/权限/指纹正确；完整链恰有五条成功 migration history；最终 `migration diff=0`。
...
- 首迁移创建的无登录 `anon`、`authenticated` 对应用表的读写均被拒绝；`service_role` 未进入任何运行时或前端配置，三端无法绕过 NestJS。
- API/Worker 的 runtime 账号无 DDL、无 `BYPASSRLS`、不拥有应用表；migration 账号未出现在运行时环境。远程连接必须 `sslmode=verify-full`。
- `pg_dump` 只走 `mall_migrator` 的 `DIRECT_URL`，TencentDB 自动备份 / PITR 必须开启，staging 恢复演练可重建角色权限、RLS 和不可变流水复算结果。
...
CH-022/CH-023 数据库结论：现有 76 models / 59 enums 足以承载 B10，不增表、列或枚举。0003 只增加两个支付事实条件唯一索引；0004 只替换触发器函数并保持 SECURITY INVOKER 和运行角色最小权限。B10.5/B10.6 不新增迁移；B10.6 总验收已通过完整 `0001→0004` 回放、20 个 partial indexes、冻结哈希、Prisma validate、`migration diff=0` 及 B10 数据库 85/85。TR-026、QA-005、`PAYMENT_TIMEOUT` 和跨提交恢复风险已关闭。最终 SHA `f5e59169b53a97704711c3aae3049e5b5d16a930` 的[普通 CI Run 33305811318](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33305811318)与[Supabase development rollback-only smoke Run 33306877575](https://github.com/wuyu222dii/Frontend-ToC-Portal-Admin-Backend-Management-System-for-Private-Domain-Mall/actions/runs/33306877575)同 SHA 且均成功，B10 development `GO`，CH-021 已自动失效；0004 作为既有前向迁移保留，真实支付、staging 与 production 仍为 `NO-GO`。

CH-024 数据库结论：契约统计保持 `173/198/198/326/705/2,695/0`，现有 76 models / 59 enums 与 `0001→0004` 足以承载 B11，数据库变更数固定为 0。B11.0-B11.5 实现与门禁已完成；B11.5 DB full **5 files / 76 passed**，受控 Supabase rollback 本地执行 **2 files / 10 passed / 3 full-only skips**（非远端 workflow 证据）。completion fixture 全表枚举、动态 Outbox aggregate 清理、PUBLISHED 业务规则恢复和事务外零残留已闭合；B11.2-B11.4 历史证据和 P2 记录保留。最终 SHA `c8e42c44a6a31099c00070294a85f628913165df` 的远端同 SHA 双绿已闭合，B11 development `GO`，CH-025 已自动失效。

CH-026 数据库结论：现有 76 models / 59 enums 足以承载 B12，Prisma 逻辑模型不变；0005 只增加退款 attempt 唯一事实和可延迟包络/金额防护，并修复 runtime 读取不可变验货事实的锁权限冲突。本地 `0001→0005` 回放、重复/跨订单项事实原子失败、合法/非法 `mall_runtime` DML、同 refund 父行并发串行、B10 迟到退款兼容、Prisma validate、冻结哈希与 `migration diff=0` 已通过；远端精确 B12.0 SHA 的普通 CI、development migration、rollback-only smoke 也已成功。B12.1-B12.6 未修改 Prisma schema 或既有迁移，数据库冻结结论继续有效；B12 full DB `65/65`、完整真实纵向、11 类数据库事实归零及测试数据库/连接精确清理通过。最终实现 SHA `8c3589afcf7bb0dd5a4b8711d418e4c61b1ad09c` 的普通 CI `33592754575` 与 Supabase rollback-only smoke `33594513127` 同 SHA 双绿，B12 development `GO`，CH-027 已自动失效；B12 orphan `P2=1` 继续阻断 staging/真实数据。
