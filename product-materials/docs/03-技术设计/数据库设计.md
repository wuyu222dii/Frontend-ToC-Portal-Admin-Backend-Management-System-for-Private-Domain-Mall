...
|---|---|
| 文档版本 | v2.4.7 |
| 对应基线 | MVP/PRD v2.4.7、CH-001 至 CH-020 |
| 数据库 | Supabase 托管 PostgreSQL，生产时间统一为 UTC |
| ORM | Prisma 7.9.1，76 个实体、59 个枚举的字段级逻辑草案见 `schema.prisma` |
| 状态 | B0-B8 development 已完成；B8 最终 SHA 已同 SHA 双绿，CH-017 已失效；CH-019/CH-020 已批准；B9.0-B9.5 本地实现与验收已完成，三项原 P1 已关闭，最终本地独立复审为 `P0=0/P1=0/P2=1`，等待最终实现同一 SHA 远端双绿；B9 development 尚未 `GO`，CH-019 仍有效，staging/production 未批准 |
| 更新时间 | 2026-08-28 |
...
当前 `schema.prisma` 已按 PostgreSQL `public` schema 表达 76 个实体和 59 个枚举。`migrations/0001_initial/migration.sql` 由 Prisma 7.9.1 基线 diff 生成后追加 PostgreSQL 专属约束，继续逐字节冻结；CH-020 只在其后增加 `0002_b9_inventory_fact_indexes`。完整链目标保持 76 张表、165 个 `CHECK`、76 张表启用 RLS、76 个 runtime policy、24 个用户触发器（45 个事件绑定），条件唯一索引由 17 增至 18；`mall_migrator` 继续拥有 76 张应用表、59 个枚举和 15 个迁移函数。最终数量、指纹和 migration diff 必须由 B9.0 空库回放实测，不得直接连接 production 验证。
...
本项目不启用 Prisma `partialIndexes` preview；条件唯一、跨字段校验、RLS 和角色权限统一在 PostgreSQL SQL migration 中管理。冻结的 `0001_initial` 共有 17 个条件唯一索引；CH-020 的 `0002_b9_inventory_fact_indexes` 在历史重复事实预检通过后增加第 18 个条件唯一索引，并增加一个 Prisma 可表达的普通复合索引，不引入辅助字段：
