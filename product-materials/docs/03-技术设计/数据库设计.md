...
| 状态 | B5 已完成并取得普通 CI/Supabase rollback-only 同 SHA 双绿；CH-014 未修改逻辑模型或首迁移，B6.0 冻结检查、Prisma validate 和 migration diff=0 已通过；B6.1 未开始，staging/production 未批准 |
| 更新时间 | 2026-08-25 |
...
- B6.1 只读查询使用独立 `StoreCatalogRepository` 和 Repeatable Read 快照。公开 Product 需 `product/brand/category` 均 ACTIVE、Product 未软删且至少一个 ACTIVE SKU；不以库存大于 0 作为可见条件。
...
- CH-014 仅增加应用只读查询口径，结构变更数为 0。B6.0 已实测 Prisma 与两份 `0001_initial` 逐字节不变，Prisma validate 通过，migration diff 为 `No difference detected`。
