...
| 状态 | B0-B10 development 已完成并维持 `GO`；B11.0 的 CH-024 契约与零迁移数据库门禁已通过并暂停，`P0=0/P1=0`，B11 总体不得 `GO`。两份 Prisma schema 与 `0001` 至 `0004` 已验证逐字节不变；CH-025 未批准前 B11.1 为 `NO-GO`。staging、production、真实支付与第三方物流均为 `NO-GO` |
| 更新时间 | 2026-08-30 |
...
- 多包裹、第三方物流 Provider、自动确认收货和普通售后属于后续独立变更；CH-024 不预建数据库结构。CH-025 未批准前不得开始 B11.1 数据访问实现。
