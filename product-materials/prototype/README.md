...
当前产品/API 基线为 `v2.4.10/CH-026`、OpenAPI `2.4.10-ch026`。B0-B11 development `GO`；B11 最终同 SHA 普通 CI 与 Supabase rollback-only smoke 双绿已闭合，CH-025 已自动失效。B12.0 只同步治理、契约、`0005` 与既有 MP-13/14、ADM-12/13/16 售后画板说明，普通售后业务尚未实现；B12.1 在精确 SHA 三项远端门禁和独立 reviewer 或另行批准 CH-027 前保持阻塞。本目录不构成 B12 工程能力、真实客户数据、真实物流/资金链路或 staging/production 的验收证据。
...
本目录仍是纯静态交互参考，不单独证明 B3 至 B11 的业务实现验收，也不证明 B12 普通售后业务已经实现。B10.0 至 B10.6 的工程与远端证据记录在阶段文档和对应 workflow；最终同 SHA 双绿已经使 B10 development `GO`。B11.1-B11.5 与最终同 SHA 双绿证据也由阶段文档、repository/API、前端和纵向测试独立记录。B12.0 仅同步文字说明，本原型不能作为 B12 repository、API、Worker、工程小程序、数据库事务、真实客户数据、真实物流/支付/退款或 staging/production 证据。
...
小程序静态画板覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、个人中心、地址、收藏、手机号授权、账号注销和服务代理绑定；同时保留确认订单、MP-09 Mock 支付结果、订单、物流与售后画板。B10 已将 PAY/CANCEL、支付处理中/终态、迟到退款与人工处理同步为静态审阅参考；CH-024 补充一单一包裹、人工物流和确认收货说明，CH-026 补充本人售后、退货验货和普通退款/重试说明。真实微信支付/退款、签名/证书、真实资金、第三方物流和普通售后工程实现仍排除，不得描述为已交付。
