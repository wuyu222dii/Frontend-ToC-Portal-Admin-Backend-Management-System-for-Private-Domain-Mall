...
本目录仍是纯静态交互参考，不单独证明 B3 至 B10 的业务实现验收。B10.0 的工程与远端三项成功证据记录在阶段文档和对应 workflow；当前原型在历史契约基础上保留 CH-022 的 B10.0 支付/对账静态边界。B10.1 Provider、支付意图与 Mock Inbox 已完成本地工程验收，但本原型仍不能作为 Store API、Provider、Inbox、Worker、工程小程序、迁移或 staging/production 证据。
...
小程序静态画板覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、个人中心、地址、收藏、手机号授权、账号注销和服务代理绑定；同时保留确认订单、MP-09 Mock 支付结果、订单、物流与售后画板。B10.0 已将 PAY/CANCEL、支付处理中/终态、迟到退款与人工处理同步为静态审阅参考；B10.1 工程已完成本地验收且未修改本轮静态画板。真实微信支付、签名/证书、真实资金、物流和普通售后退款仍排除，不得描述为已交付。
