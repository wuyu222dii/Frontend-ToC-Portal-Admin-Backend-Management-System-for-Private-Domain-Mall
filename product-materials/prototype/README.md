...
当前产品/API 基线为 v2.4.4/CH-014。B5 development 已在同一实现 SHA 取得普通 CI 与 Supabase rollback-only 双绿；CH-013/CH-014 已批准，当前目录完成 B6.0 匿名目录静态交互同步。这里不包含 B6 Store repository/API 或生产小程序实现，也不替代 B6.4 最终远端 workflow、staging 或生产验收。
...
当前 v2.4.4（CH-014）覆盖 21 个小程序页面、9 个代理页面/关键视图和 22 个总部页面/关键视图。验收脚本渲染 96 个桌面/移动组合，并执行 14 条小程序流程和 18 条总部/代理流程；ADM-03/04/05/06/07/08 固定覆盖 375、390、414、1024、1440 五种目标视口，关键异步门禁以可观察状态等待代替固定毫秒延迟。消费者目录断言覆盖商品名搜索、COMPREHENSIVE/价格排序、ACTIVE 零库存 SKU、全售罄商品可浏览、首页单区失败恢复和购物车失效项保留；既有交易、安全和后台门禁继续回归。
...
本目录仍是纯静态交互参考，不单独证明 B3、B4、B5 或 B6 业务实现验收。当前原型证明 CH-010 的 ADM-03/04、CH-012 的 ADM-07/08 与 CH-014 的消费者匿名目录交互契约可落地；静态原型不能作为 Store API、工程小程序或 staging/production 证据。
...
小程序覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、确认订单、Mock 支付、四轴订单详情、物流、售后、地址、收藏、手机号授权、账号删除、个人中心和服务代理绑定。B6 交付边界只包含前六个匿名浏览/本地购物车界面；后续页面继续作为全量 MVP 原型参考，不代表已进入 B6 实施范围。
