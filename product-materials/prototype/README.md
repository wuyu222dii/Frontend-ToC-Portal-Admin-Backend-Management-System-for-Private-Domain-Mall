...
当前产品/API 基线为 v2.4.8/CH-022。B0-B9 development 已完成；B9 最终 SHA `19f9ad57190b28d11922db805b39af95b2f7ba3b` 的普通 CI Run `33230769777` 与 Supabase development rollback-only Run `33233087710` 同 SHA 双绿，B9 development `GO`。当前目录已将 MP-09、订单 PAY/CANCEL、Mock 支付状态矩阵及 ADM-10 只读对账待办同步到 B10.0 静态参考。B10.1 尚未开始；本原型不代表 Provider、Worker、迁移或远端门禁已通过。
...
B10.0 的契约统计、迁移链与三项远端门禁须由对应工程批次独立交付；静态回归不作为替代证据。
...
本目录仍是纯静态交互参考，不单独证明 B3 至 B10 的业务实现验收。B9 的工程与远端双绿证据记录在阶段文档和对应 workflow；当前原型在历史契约基础上增加 CH-022 的 B10.0 支付/对账静态边界。静态原型不能作为 Store API、Provider、Worker、工程小程序、迁移或 staging/production 证据。
...
小程序静态画板覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、个人中心、地址、收藏、手机号授权、账号注销和服务代理绑定；同时保留确认订单、MP-09 Mock 支付结果、订单、物流与售后画板。B10.0 只将 PAY/CANCEL、支付处理中/终态、迟到退款与人工处理同步为静态审阅参考。真实微信支付、签名/证书、真实资金、物流和普通售后退款仍排除，不得描述为已交付。
