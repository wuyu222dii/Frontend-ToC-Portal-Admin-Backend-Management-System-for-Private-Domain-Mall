...
当前产品/API 基线为 v2.4.5/CH-016。B6 development 已取得普通 CI 与 Supabase rollback-only 双绿；当前目录完成 B7.0 的 MP-07/18/19/20/21 静态契约同步。这里不包含消费者身份 repository/API、真实微信能力或生产小程序实现，也不替代后续 B7 工程批次、staging 或 production 验收。
...
当前 v2.4.5（CH-016）覆盖 21 个小程序页面、9 个代理页面/关键视图和 22 个总部页面/关键视图。验收脚本渲染 96 个桌面/移动组合，并执行 14 条小程序流程和 18 条总部/代理流程；ADM-03/04/05/06/07/08 固定覆盖 375、390、414、1024、1440 五种目标视口，关键异步门禁以可观察状态等待代替固定毫秒延迟。消费者断言新增当前协议 exact set 与 409 刷新、候选确认/拒绝、最小服务代理投影、手机号脱敏授权/撤回，以及注销 preview、确认重检和同步清会话；既有目录、交易、安全和后台门禁继续回归。
...
本目录仍是纯静态交互参考，不单独证明 B3 至 B7 的业务实现验收。当前原型证明 CH-010 的 ADM-03/04、CH-012 的 ADM-07/08、CH-014 的消费者匿名目录和 CH-016 的 B7 身份隐私交互契约可落地；静态原型不能作为 Store API、工程小程序或 staging/production 证据。
...
小程序覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、确认订单、Mock 支付、四轴订单详情、物流、售后、地址、收藏、手机号授权、账号注销、个人中心和服务代理绑定。B7.0 只冻结登录/协议、个人资料、服务代理、手机号与隐私注销交互；收藏、服务端购物车、地址、订单、支付、真实微信与资金链路仍是后续阶段边界，登录返回原页面也不会伪造这些动作成功。
