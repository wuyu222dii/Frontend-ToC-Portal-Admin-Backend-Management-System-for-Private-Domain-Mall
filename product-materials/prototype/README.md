...
当前产品/API 基线为 v2.4.9/CH-024。B10.0、B10.2/CH-023 及 B10.6 最终同 SHA 双绿证据继续作为历史记录保留，B10 development `GO`、CH-021 已自动失效。CH-024/CH-025 已批准；CH-025 仅适用于 B11.1-B11.5 脱敏 development，并继续有效至 B11.5 完成。B11.0 与 B11.1 本地门禁已通过，B11.1 `P0=0/P1=0` 并暂停在 B11.2 前；B11 不得标记 `GO`，尚无最终普通 CI/Supabase smoke 同 SHA 双绿。当前目录仅将既有 MP-10/11/12、ADM-09/10/11 画板解释为一单一包裹、总部人工物流、确认收货/兜底完成及佣金一次入账的静态参考，不构成 B11 工程能力、真实客户数据、真实物流/资金链路或 staging/production 的验收证据。
...
本目录仍是纯静态交互参考，不单独证明 B3 至 B11 的业务实现验收。B10.0 至 B10.6 的工程与远端证据记录在阶段文档和对应 workflow；最终同 SHA 双绿已经使 B10 development `GO`。B11.1 查询与 PII 的工程验收已由 repository/API/测试证据独立完成，本原型不能替代这些证据，也不能作为 B11.2 写路径、数据库事务、佣金结转、工程小程序、真实客户数据、真实物流/支付或 staging/production 证据。
...
小程序静态画板覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、个人中心、地址、收藏、手机号授权、账号注销和服务代理绑定；同时保留确认订单、MP-09 Mock 支付结果、订单、物流与售后画板。B10 已将 PAY/CANCEL、支付处理中/终态、迟到退款与人工处理同步为静态审阅参考；CH-024 仅补充一单一包裹、人工物流和确认收货的说明。真实微信支付、签名/证书、真实资金、第三方物流和普通售后退款仍排除，不得描述为已交付。
