...
当前产品/API 基线为 v2.4.6/CH-018。B0-B7 development 已完成；当前目录完成 B8.0 的 MP-04/06/15/16/17/18 静态契约同步。这里不包含 B8.1-B8.4 repository/API/工程小程序、真实微信能力或生产实现，也不替代后续批次、staging 或 production 验收。
...
当前 v2.4.6（CH-018）覆盖 21 个小程序页面、9 个代理页面/关键视图和 22 个总部页面/关键视图。验收脚本渲染 97 个桌面/移动组合，并执行 16 条小程序流程和 18 条总部/代理流程；覆盖 375、390、414、1024、1440 五种目标视口，关键异步门禁以可观察状态等待代替固定毫秒延迟。消费者断言包括 B7 身份隐私历史回归，以及 B8 收藏恢复一次、五状态购物车/合并 journal、地址版本冲突与默认约束；既有目录、交易、安全和后台门禁继续回归。
...
本目录仍是纯静态交互参考，不单独证明 B3 至 B8 的业务实现验收。当前原型证明 CH-010 的 ADM-03/04、CH-012 的 ADM-07/08、CH-014 的消费者匿名目录、CH-016 的 B7 身份隐私和 CH-018 的 B8 登录后购物基础交互契约可落地；静态原型不能作为 Store API、工程小程序或 staging/production 证据。
...
小程序静态画板覆盖首页、分类、搜索、商品详情、SKU 选择、购物车、个人中心、地址、收藏、手机号授权、账号注销和服务代理绑定；同时保留确认订单、Mock 支付、订单、物流与售后的未来阶段画板供跨阶段评审。B8 主流程只开放收藏、服务端购物车、游客登录合并和地址，登录收藏只恢复一次；结算、订单、支付、真实微信与资金链路继续排除，未来画板只能从原型导航直达，不作为 B8 已实现证据。
