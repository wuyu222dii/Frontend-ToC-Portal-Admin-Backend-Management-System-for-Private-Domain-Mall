...
- 可交互参考：`product-materials/prototype/agent.html`
...
CH-005 以 9 个代理页面/关键视图为冻结契约：登录、7 个工作台 `.page-view` 和商品分享弹层。登录不提供固定账号或密码，采用模拟服务端格式校验，并覆盖失败、停用、连续 5 次锁定 15 分钟与首次强制改密。所有原始订单项佣金只读支付时快照，不得按当前商品规则回算。

v1.4 不增加代理端页面；总部代理详情现以当前 agentId 下钻客户、已付归因订单、佣金、钱包、提现和目标审计。这些总部投影与本工作台使用同一 agentId 隔离边界，不得因总部下钻扩大代理端权限。
...
- `Overlay/Bank`：账户姓名、开户银行、银行卡号；执行必填、姓名、银行和卡号格式校验。提交后仅保留后四位，立即清除完整输入并显示加密说明。
...
- 账号和密码默认均为空；不展示、预填或提示任何固定正确值，提交交由模拟服务端按格式和账号状态校验。
- 错误凭据、停用账号和连续 5 次失败锁定 15 分钟均有独立反馈；首次有效登录连接 `Login/First-Password`，修改成功后进入看板。
...
- 从当前客户详情进入订单时自动带入该客户筛选，结果不得混入其他代理客户或平台客户。
...
- 提交按钮在请求期间禁用并携带幂等语义；重复提交不得新增第二笔提现，存在进行中申请时入口直接禁用并说明原因。
...
- 银行卡更新完成后页面只显示开户人、银行和卡号后四位；完整卡号不得进入列表、详情、日志或页面文本。
...
`Login/Default` → 模拟服务端校验 → 失败/停用/限流或 `Login/First-Password` → 设置新密码 → `Dashboard/Default`。
...
`Commission/List` → 查看钱包 → `Wallet/Default` → 无进行中申请时申请提现 → `Wallet/Withdraw-Modal` → 防重复提交 → 列表只新增一条“审核中”记录 → `Wallet/Withdrawal-Drawer`。
...
`Account/Default` → 修改密码 → `Account/Password-Modal`；管理银行卡 → `Account/Bank-Modal` → 格式校验 → 保存后仅写回后四位并清除完整输入。
...
- [ ] 登录默认无凭据或正确值提示；错误、停用、5 次失败锁定和首次强制改密均可演示。
...
- [ ] 同一代理只允许一笔进行中提现，重复提交不新增记录；银行卡保存后完整输入立即清除且页面仅显示后四位。
