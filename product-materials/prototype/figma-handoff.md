...
CH-005 的 v1.5 当时未新增页面，在原视图内补齐 SPU 库存汇总/SKU 下钻、验货证据封存与 typed 二阶段处置、代理六类目标下钻。当前 v1.6 继续保持 22 个总部页面/关键视图，只同步 ADM-05/06 的 CH-006 契约交互。所有详情、弹层和抽屉绑定当前选中记录。
