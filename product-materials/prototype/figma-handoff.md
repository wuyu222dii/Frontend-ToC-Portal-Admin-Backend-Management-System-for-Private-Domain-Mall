...
| `Admin/Products/Edit-SKU` | 1440 × 1024 | 规格、零售价、只读库存、推荐状态与生命周期 |
...
- 顶部主动作仅保留“新增商品”。
- 状态 Tab：默认列表、已启用、已停用、草稿；已归档记录只通过显式筛选查看。
- 筛选器：关键词、一级分类、品牌、状态、重置。
- 表格字段：商品、最低活动价、SKU 数及实物/锁定/可售只读汇总、净销量、状态/首次发布、商品标记和操作。没有已启用 SKU 时，最低活动价显示“暂无活动价”。
- 启用、停用和归档必须先打开影响预览再确认；已启用商品不显示直接归档入口。
- 筛选无结果时显示空状态，不改变表格容器高度。

### 5.4 商品编辑与 SKU

- 页面采用主内容 + 260px 只读摘要侧栏；1024 侧栏收窄至 230。
- Tab：基本信息、SKU 与价格、详情内容。
- 基本信息：图片、商品名称、品牌、货号、一级分类、商品卖点。
- SKU：规格名称、不可变编码、零售价、实物/锁定/可售只读库存、SKU 推荐、状态/版本与独立生命周期操作。创建 SKU 同时创建 0/0 库存余额，不在 ADM-04 调整库存。
- 商品固定以草稿创建，SKU 固定以已停用创建；普通资料保存不改变状态。货号和 SKU 编码创建后不可修改，软归档后仍保留。
- 图集最多 8 张并稳定排序；商品详情保留全部 SKU，包括已归档 SKU。
- 详情内容：商品介绍、成分说明、使用方法，可新增图文模块。
- 侧栏：热销/新品标记、图集、SKU 数、实物/锁定库存和首次发布时间；不放入普通状态下拉、库存调整、佣金或“保存并上架”。
- 恢复商品固定回到草稿，恢复 SKU 固定回到已停用；恢复后必须重新预览才能启用。

### 5.4A 商品准入阻断与并发冲突

- 启用商品时，缺少公开图片提供“去上传商品图片”，缺少已启用 SKU 提供“去管理 SKU”。
- 归档商品时，存在已启用 SKU 提供“去停用 SKU”；存在活动库存预占时，按支付/售后来源提供“查看相关订单/售后”，只读定位占用来源，不导向可写库存调整。
- 四类 422 主提示只显示中文原因与修复入口；`PRODUCT_PRIMARY_IMAGE_REQUIRED`、`PRODUCT_ACTIVE_SKU_REQUIRED`、`ACTIVE_SKU_DEPENDENCY`、`ACTIVE_INVENTORY_RESERVATION` 及 HTTP 状态只放入默认收起的诊断详情和 DOM data 属性。
- Product 或 SKU 版本冲突时，界面立即刷新最新资源、销毁旧 preview、取消勾选并禁用旧确认。“返回最新商品/SKU”只导航到新版本，管理员必须重新发起 preview/confirm，不得自动覆盖、自动重试或复用旧 token。
- Product 启用时品牌或分类非已启用状态，折叠诊断固定记录 `STATE_CONFLICT` 409，不得误标为上述 Product 422。

### 5.4B 品牌、分类、Banner 与库存
...
- 参考画板：`product-materials/prototype/index.html?screen=service-agent&device=375`；未绑定邀请确认使用 `&binding=unbound&invite=QX-A1038`。
...
- [ ] 品牌/分类创建固定为草稿，排序接受 0 并拒绝负数与小数，普通编辑不提供状态控件。
- [ ] 三类生命周期操作均展示影响预览；活动商品依赖时仍可提交确认，并展示 `ACTIVE_PRODUCT_DEPENDENCY` 422，记录与版本不变且允许关闭或重试。
- [ ] 默认列表排除归档，显式归档筛选可恢复为草稿，恢复后须单独启用。
