...
- 客户手机号独立、可空且仅来自客户自愿微信授权；已授权仅显示后四位，未授权显示“手机号未绑定”，不展示真实姓名和详细收货地址。
- 全体代理共享平台商品佣金规则，不展示或编辑代理个人比例。
- 有效比例遵循“SKU 覆盖优先于分类默认”，订单项需同时展示 SKU、比例和规则来源快照。
- 归属代理、计算基数、有效比例、规则来源和金额均展示支付时快照语义。
- 原始预计/入账流水不可修改；完成前退款追加 `CANCELLED`，完成后退款追加 `REFUND_DEBIT`，正向与负向记录并列展示。
...
| `Agent/Wallet/Negative` | 1440 × 1000 | 负余额、提现禁用与后续佣金优先抵扣 |
| `Agent/Wallet/Withdraw-Modal` | 1440 × 1000 | 金额、银行卡和确认声明 |
| `Agent/Wallet/Withdrawal-Drawer` | 1440 × 1000 | 审核、打款和凭证时间线 |
| `Agent/Wallet/Withdrawal-Approved` | 1440 × 1000 | APPROVED / 已审核待打款状态 |
...
| `Agent/States/Loading-Error-403-409-Success` | 1440 × 1000 | 代表性反馈与权限/冲突状态 |

在 `03 Agent Desktop 1024` 中复制登录、看板、商品、客户、订单、佣金、钱包和账户 8 个默认画板，并补充负余额与 APPROVED 提现；在 `04 Agent Mobile 390` 中复制同样页面及分享、提现、订单详情、负余额四个移动状态。底部导航固定为首页、推广、订单、佣金、我的。
...
- `Product/Share Card`：商品图、品牌/分类、默认推广 SKU、平台零售价、预计佣金、规则来源、平台销量、库存状态、查看和推广。
- `Customer/Masked Row`：昵称、手机后四位、城市、绑定时间、归属消费、次数和最近商品。
- `Order/Attributed Row`：订单号、脱敏客户、商品、实付、订单状态、佣金、规则快照摘要及佣金状态。
- `Commission/Ledger Row`：时间、类型、订单/SKU 快照、净实付、有效比例、规则来源、变动金额和余额影响。
- `Commission/Rule Source`：`分类默认` 使用绿色状态标签，`SKU 覆盖` 使用蓝色状态标签；仅作只读来源说明。
- `Commission/SKU Rule Item`：规格、SKU 编码、平台零售价、有效比例、规则来源和单件预计佣金。
- `Withdrawal/Row`：提现单、时间、金额、脱敏银行卡、状态、完成时间和详情。
- `Status/Tag`：预计为琥珀、可提现/完成为绿、运输中/APPROVED 为蓝、`REFUND_DEBIT`/驳回为红、`CANCELLED` 为中性灰。
...
- 指标：今日归属销售额、今日归属订单、绑定客户、佣金规则覆盖。
- 规则覆盖采用“分类数 · SKU 数”主值，并在辅助文案说明“分类默认 + SKU 覆盖”；预计佣金仍在佣金账户卡展示。
...
- 摘要：授权商品数、佣金规则覆盖、本月访问、本月归属订单。
- 筛选：关键词、分类、热销/佣金/新品排序。
- 商品卡展示默认推广 SKU 的零售价、预计佣金金额、有效比例与“分类默认 / SKU 覆盖”来源。
- 商品详情按 SKU 分组展示规格、编码、零售价、有效比例、规则来源和单件预计佣金。
- 商品卡主动作是“立即推广”，次动作是“查看”；不显示编辑价格、库存或佣金入口。
- 分享弹层支持商城级和商品级两类链接，均携带 `QY6088` 归属参数。

### 6.4 客户与订单

- 客户列表只出现微信昵称、可选手机号尾号、城市和运营统计；未授权手机号显示“手机号未绑定”，不得从收货地址推导。
- 客户详情明确展示长期绑定关系；不展示解绑、转移或编辑动作。
- 订单列表按待发货、运输中、已完成和退款售后筛选。
- 订单详情按订单项显示支付时 SKU、净实付、有效比例、规则来源和佣金快照；混合订单允许存在不同的有效比例。
- 售后订单显示平台处理时间线；订单项始终展示原始正向佣金，另列 `CANCELLED` 或 `REFUND_DEBIT`，不得把原快照改成负数。

### 6.5 佣金与钱包

- 佣金列表覆盖 `ESTIMATE`、`CREDIT`、`CANCELLED`、`REFUND_DEBIT` 四类流水，每条对应一个订单项 SKU。
- 列表和移动数据卡同时显示有效比例与“分类默认 / SKU 覆盖”规则来源。
- 佣金详情展示 `订单项净实付 × 支付时有效比例 = 佣金`，并列出流水类型、SKU 和规则来源冻结快照；正向原始记录与冲正记录并列存在。
- 钱包区分可提现、预计、处理中、累计提现和负余额；负余额状态禁用提现，后续 CREDIT 优先抵扣。
- 提现最低金额为 100 元、手续费为 0；提交后进入审核中状态。同一代理同时只能有一笔待审核或已审核未打款申请。
- 提现详情按提交、审核、线下打款、凭证上传展示时间线，并必须包含 `APPROVED / 已审核待打款` 示例。

### 6.6 账户

- 资料区包含代理编号、邀请码、开通时间和“统一商品规则”。
- 规则说明明确“按订单项 SKU 支付时有效比例核算，代理端仅可查看”。
- 资料修改采用申请语义；密码和银行卡可独立进入弹层。
- 权限标签只显示推广商品、查看客户、查看订单和佣金提现。
- “我的”页面提供我的客户与我的钱包入口；移动底部导航固定 5 项，客户页归属“我的”、钱包页归属“佣金”，使用 `aria-current` 同步当前项。
- 账户页提供 Loading、网络 Error、403、409 与 Success 五种代表状态，用于统一研发反馈组件。
...
账务分支：原 `CREDIT` / `ESTIMATE` 详情 → 同订单独立 `REFUND_DEBIT` / `CANCELLED` 详情，原记录内容保持不变。

### Flow D · 佣金提现

`Commission/List` → 查看钱包 → `Wallet/Default` → 申请提现 → `Wallet/Withdraw-Modal` → 提交 → 列表新增“审核中”记录 → `Wallet/Withdrawal-Drawer`。

负余额分支：`Wallet/Default` → 切换“负余额示例” → `Wallet/Negative` → 提现禁用 → 查看佣金抵扣说明。

审核分支：提现记录 → `Wallet/Withdrawal-Approved` → 查看等待平台线下打款时间线。
...
- [ ] 未授权客户显示“手机号未绑定”，且原型无从收货地址补写账户手机号的路径。
- [ ] 所有商品价格、库存、履约和售后均为平台统一管理语义。
- [ ] 看板、选品和账户页均使用统一商品规则语义，不提供佣金编辑入口。
- [ ] 商品卡和商品详情同时出现不同有效比例，并标明“分类默认 / SKU 覆盖”。
- [ ] 佣金比例、规则来源、SKU、基数、金额和归属代理采用支付时快照语义。
- [ ] `CREDIT` 与 `REFUND_DEBIT`、`ESTIMATE` 与 `CANCELLED` 可并列查看，原正向记录不被覆盖或改负。
- [ ] 钱包负余额状态、提现禁用、后续佣金优先抵扣和 APPROVED 提现记录可点击演示。
- [ ] 移动导航固定 5 项，“我的”可进入客户，“佣金”可进入钱包，当前项状态一致。
- [ ] Loading、Error、403、409 和 Success 代表状态可切换。
