...
- 使用角色：超级管理员（总部平台全部权限）
- 首期页面：登录、数据看板、商品、订单、售后、客户、代理管理、客户归属调整、提现审核与打款凭证
...
| `Admin/Agents/Create-Modal` | 1440 × 1024 | 账号、佣金比例与临时密码 |
| `Admin/Agents/Edit-Modal` | 1440 × 1024 | 修改资料、比例和状态 |
...
在 `03 Admin Desktop 1024` 中复制以下 10 个关键画板并调整自适应：登录、看板、商品列表、商品编辑 SKU、订单列表、订单详情、售后列表、客户列表、代理列表、提现列表。
