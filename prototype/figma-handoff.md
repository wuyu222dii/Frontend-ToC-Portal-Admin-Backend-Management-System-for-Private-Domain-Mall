...
| `Admin/Withdrawals/Bank-Reveal` | 1440 × 1024 | APPROVED 提现二次验证、60 秒授权与失败锁定 |
| `Admin/BusinessRules/Default` | 1440 × 1024 | CH-003 当前业务规则版本 |
| `Admin/AuditLogs/List` | 1440 × 1024 | 高风险操作不可变日志 |
| `Admin/States/Loading-Error-403-409-Success` | 1440 × 1024 | 通用系统反馈状态 |
| `Admin/Common/High-Risk-Confirm` | 1440 × 1024 | 原因、影响范围、确认勾选与审计反馈 |
...
- 拒绝与同意退款均必须填写处理说明，经影响预览和二次确认后执行；驳回释放占用，退款失败保留占用供重试。
...
- 待审核可通过或拒绝；拒绝必须填写原因、展示金额影响并二次确认，冻结金额按负余额优先规则返回钱包。
- 通过后进入待打款，必须上传线下银行转账凭证才能标记已打款。
- 银行卡仅显示后四位；原型不实施真实文件上传或自动打款。
- 完整卡号入口只在内部状态 `APPROVED`（界面“待打款”）出现；验证授权绑定当前管理员和提现单，仅使用一次并在 60 秒或离页时清除。
- 二次验证连续失败 5 次锁定 15 分钟；Error、Locked、Success 均独立成画板并写入安全审计。
- 拒绝时冻结额返回可提现并优先抵扣负余额；已打款时扣减冻结额，两者均同步钱包分项。

### 5.11 业务规则、审计与系统状态

- 业务规则页集中展示支付超时、迟到支付退款、未发货回库、售后占用、商品白名单、手机号和账号删除规则版本。
- 审计日志只追加，字段包括时间、编号、事件、操作人、目标、原因、结果、客户端与状态。
- 高风险操作统一使用原因、影响范围、确认勾选和成功审计反馈，不允许仅依赖 Toast 完成确认。
...
敏感查看分支：`Withdrawals/List[APPROVED]` → `Withdrawals/Bank-Reveal` → 二次验证 → 60 秒单次展示；失败 5 次进入 Locked，离页立即恢复掩码。
...
治理分支：高风险操作 → `Common/High-Risk-Confirm` → 原因 + 影响 + 确认 → `AuditLogs/List`；版本冲突进入 `States/409`。
...
- [ ] 完整银行卡入口仅在 APPROVED 提现出现，60 秒单次授权、离页清除和 5 次失败锁定可演示。
- [ ] 代理停用、客户转移、售后拒绝/退款、提现拒绝和佣金规则变更均具备原因、影响、确认与审计反馈。
