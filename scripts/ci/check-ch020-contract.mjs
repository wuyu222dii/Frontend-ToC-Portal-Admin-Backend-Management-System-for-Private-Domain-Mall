...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'qingxu-ch020-contract-'));
...
try {
  ...
  assert.equal(document.info.version, '2.4.7-ch020');
  ...
  assert.equal(Object.keys(schemas).length, 325, 'OpenAPI schema count drifted');
  ...
  assert.match(customerRateLimit.description,
    /13 个.+5 个报价\/订单 operation.+CUSTOMER.+HMAC.+每 60 秒最多 120 次/s);
  ...
  assert.match(orderListOperation.description, /不返回 PAY/);
  ...
  }, 'B9 order lists must expose only the CANCEL action');
  ...
  assert.deepEqual(storeOrderDetail.properties.available_actions.items, {
    type: 'string', const: 'CANCEL',
  }, 'B9 order details must expose only the CANCEL action');
  for (const field of ['packages', 'aftersales', 'payment_attempts', 'refund_attempts']) {
    assert.equal(storeOrderDetail.properties[field].maxItems, 0,
      `B9 order detail ${field} must remain empty until its later phase opens`);
  }
  ...
  assert.equal(schemaReferences, 703, 'OpenAPI schema reference count drifted');
  assert.equal(references.length, 2_665, 'OpenAPI local reference count drifted');
