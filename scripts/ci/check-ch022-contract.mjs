...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'qingxu-ch022-contract-'));
...
try {
  ...
  assert.equal(document.info.version, '2.4.8-ch022');
  ...
  assert.match(customerRateLimit.description,
    /13 个.+5 个报价\/订单 operation.+2 个支付 operation.+CUSTOMER.+HMAC.+每 60 秒最多 120 次/s);
  ...
  assert.deepEqual(schemas.StoreOrderListItem.properties.available_actions.items, {
    type: 'string', enum: ['PAY', 'CANCEL'],
  }, 'B10 order lists must expose only PAY and CANCEL actions');
  ...
  assert.deepEqual(storeOrderDetail.properties.available_actions.items, {
    type: 'string', enum: ['PAY', 'CANCEL'],
  }, 'B10 order details must expose only PAY and CANCEL actions');
  assert.match(storeOrderDetail.properties.available_actions.description,
    /无支付意图.+失败终态 intent.+CANCEL/s);
  for (const field of ['packages', 'aftersales']) {
    assert.equal(storeOrderDetail.properties[field].maxItems, 0,
      `B10 order detail ${field} must remain empty until its later phase opens`);
  }
  ...
  assert.equal(references.length, 2_678, 'OpenAPI local reference count drifted');
