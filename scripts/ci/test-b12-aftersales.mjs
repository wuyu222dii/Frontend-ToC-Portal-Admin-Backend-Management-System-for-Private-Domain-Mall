...
const mode = process.env.B12_STORE_AFTERSALE_DATABASE_TEST_MODE;

function fail(message) {
  process.stderr.write(`B12 aftersales database test refused: ${message}\n`);
  process.exit(1);
}
...
if (mode !== 'full' && mode !== 'rollback') {
  fail('B12_STORE_AFTERSALE_DATABASE_TEST_MODE must be explicitly set to full or rollback');
}
...
process.env.B12_STORE_AFTERSALE_DATABASE_TEST_MODE = mode;
...
process.stdout.write(`B12 aftersales ${mode} database checks passed.\n`);
