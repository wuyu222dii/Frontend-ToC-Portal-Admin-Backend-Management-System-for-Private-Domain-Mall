...
function fail(message) {
  process.stderr.write(`B2 database test refused: ${message}\n`);
  process.exit(1);
}

if (mode !== 'full' && mode !== 'rollback') {
  fail('B2_DATABASE_TEST_MODE must be explicitly set to full or rollback');
}
...
for (const command of [
  ...
  ['--filter', '@qingxu/database', 'exec', 'vitest', 'run', 'src/admin-auth.integration.spec.ts'],
]) {
