...
function fail(message) {
  process.stderr.write(`B3 catalog API integration test refused: ${message}\n`);
  process.exit(1);
}
...
if (process.env.B3_CATALOG_API_TEST_MODE !== 'full') {
  fail('B3_CATALOG_API_TEST_MODE must be explicitly set to full');
}
...
if (process.env.S3_FORCE_PATH_STYLE !== 'true') {
  fail('S3_FORCE_PATH_STYLE must be true for the isolated local storage configuration');
}
...
for (const command of [
  ...
  ['--filter', '@qingxu/api', 'exec', 'vitest', 'run', 'src/admin-catalog/admin-catalog.integration.spec.ts'],
]) {
...
process.stdout.write('B3.2 catalog API integration checks passed.\n');
