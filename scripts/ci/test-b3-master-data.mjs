...
const mode = process.env.B3_MASTER_DATA_DATABASE_TEST_MODE;

function fail(message) {
  process.stderr.write(`B3 master-data database test refused: ${message}\n`);
  process.exit(1);
}
...
if (mode !== 'full' && mode !== 'rollback') {
  fail('B3_MASTER_DATA_DATABASE_TEST_MODE must be explicitly set to full or rollback');
}
...
if (mode === 'full') {
  ...
  if (!['postgres:', 'postgresql:'].includes(url.protocol) || !LOCAL_HOSTS.has(url.hostname) ||
    ...
    !/(?:^|[-_])(?:b3|test|ephemeral)(?:[-_]|$)/i.test(database)) {
...
for (const command of [
  ...
  ['--filter', '@qingxu/database', 'exec', 'vitest', 'run', 'src/master-data.integration.spec.ts'],
]) {
...
process.stdout.write('B3.2 master-data database checks passed.\n');
