...
const mode = process.env.B4_PRODUCT_CATALOG_DATABASE_TEST_MODE;

function fail(message) {
  process.stderr.write(`B4 product-catalog database test refused: ${message}\n`);
  process.exit(1);
}
...
if (mode !== 'full' && mode !== 'rollback') {
  fail('B4_PRODUCT_CATALOG_DATABASE_TEST_MODE must be explicitly set to full or rollback');
}
...
if (mode === 'full') {
  ...
  if (!['postgres:', 'postgresql:'].includes(url.protocol) || !LOCAL_HOSTS.has(url.hostname) ||
    ...
    !/(?:^|[-_])(?:b4|test|ephemeral)(?:[-_]|$)/i.test(database)) {
    fail('full mode requires a query-free loopback mall_runtime B4 test database');
  }
} else {
