...
const mode = process.env.B5_INVENTORY_DATABASE_TEST_MODE;

function fail(message) {
  process.stderr.write(`B5 Inventory database test refused: ${message}\n`);
  process.exit(1);
}
...
if (mode !== 'full' && mode !== 'rollback') {
  fail('B5_INVENTORY_DATABASE_TEST_MODE must be explicitly set to full or rollback');
}
...
if (mode === 'full') {
  ...
  if (!['postgres:', 'postgresql:'].includes(url.protocol) || !LOCAL_HOSTS.has(url.hostname) ||
    ...
    !/(?:^|[-_])(?:b5|inventory|test|ephemeral)(?:[-_]|$)/i.test(database)) {
    fail('full mode requires a query-free loopback mall_runtime B5 Inventory test database');
  }
} else {
...
process.env.B5_INVENTORY_API_TEST_MODE = mode;

for (const command of [
  ['--filter', '@qingxu/platform-core', 'build'],
  ['--filter', '@qingxu/config', 'build'],
  ['--filter', '@qingxu/database', 'build'],
  ['--filter', '@qingxu/database', 'exec', 'vitest', 'run', 'src/inventory.integration.spec.ts'],
  ['--filter', '@qingxu/api', 'exec', 'vitest', 'run', 'src/admin-inventory/admin-inventory.integration.spec.ts'],
]) {
...
process.stdout.write('B5.2 Inventory database and API checks passed.\n');
