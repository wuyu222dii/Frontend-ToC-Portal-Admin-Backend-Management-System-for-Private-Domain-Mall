...
const mode = process.env.B6_STORE_CATALOG_DATABASE_TEST_MODE;

function fail(message) {
  process.stderr.write(`B6 Store catalog database test refused: ${message}\n`);
  process.exit(1);
}
...
if (mode !== 'full' && mode !== 'rollback') {
  fail('B6_STORE_CATALOG_DATABASE_TEST_MODE must be explicitly set to full or rollback');
}
...
if (mode === 'full') {
  ...
  if (!['postgres:', 'postgresql:'].includes(url.protocol) || !LOCAL_HOSTS.has(url.hostname) ||
    ...
    !/(?:^|[-_])(?:b6|store|catalog|test|ephemeral)(?:[-_]|$)/i.test(database)) {
    fail('full mode requires a query-free loopback mall_runtime B6 test database');
  }
} else {
...
process.env.B6_STORE_CATALOG_DATABASE_TEST_MODE = mode;
...
for (const command of [
  ...
  ['--filter', '@qingxu/database', 'exec', 'vitest', 'run', 'src/store-catalog.integration.spec.ts'],
]) {
...
process.stdout.write('B6.1 Store catalog database checks passed.\n');
